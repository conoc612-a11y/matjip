...
### 1) 최적화 2번 — `realprice_apt.json` 복구 **[미완, 설계 결정 필요]**
현재 상태: **아무것도 안 바뀜.** 수집 시도했으나 가드가 막아서 중단했고, 기존 데이터는 전부 무사.

**막힌 이유(중요):** 새 수집기 `collect_realprice.js` 는 **서울 25개 구만** 대상인데, 지금 쓰는
폴백 `realprice_seoul_gg.json`(9,030건)은 **서울+경기 65개 지역**이고 그중 **경기가 4,857건
(53.8%)** 이다. 서울만 받아서 `realprice_apt.json` 을 채우면 프론트가 그걸 우선 쓰므로
**경기 데이터가 통째로 사라진다**(수원·성남·안양·평택 등). 사용자는 "서울/경기 모두 필요"라고 확인함.

**정정 사항:** 이전에 "2MB 를 헛으로 받는다"고 했던 건 틀렸다. 그 2MB 는 실제로 쓰이는
데이터다. 진짜 낭비는 빈 파일(5바이트)을 먼저 받아보는 요청 1회뿐. 따라서 2번의 실익은
용량 절감이 아니라 **죽어 있는 기능 복구**(전세가율·직거래·계약해제 표시, 정확한 좌표)다.

**합의된 방향:** 수집기에 경기 40개 시군구를 추가한 뒤 `MONTHS=12 WITH_APT=1` 로 한 번에 수집.
(3개월로 돌리면 villa/officel 기존 12개월치가 축소돼 가드에 막힌다.)

**진행하다 만 것:** 경기 법정동코드 42개를 매핑해 검증 스크립트까지 작성했으나 **실행 전 중단**.
스크립트: `%TEMP%\claude\...\scratchpad\verify-gg-codes.cjs` (세션 임시폴더라 사라졌을 수 있음 —
없으면 다시 작성). 코드를 추측으로 넣지 말고 **API 로 1건씩 검증한 뒤** `GU_ALL` 에 넣을 것.

**주의:**
- `WITH_APT=1` 이 없으면 아파트를 아예 수집하지 않는다(`collect_realprice.js:482`). 이게
  `realprice_apt.json` 이 계속 비어 있던 진짜 이유.
- 수집 실행: `DGK=<키> MONTHS=12 WITH_APT=1 node tools/collect_realprice.js`
  키는 `keys.env` 의 `DGK=` 에 사용자가 넣어둠(96자, Decoding 키, 인증 확인 완료).
- 지오코딩이 오래 걸린다(5,034건에 약 25분, V-World 차단 재시도 포함). 12개월 + 경기면 훨씬 김.
- `SUFFIX=_test` 를 주면 실제 파일을 안 덮어쓰고 시험할 수 있다. **먼저 이걸로 검증 권장.**
