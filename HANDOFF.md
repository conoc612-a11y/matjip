...
> 📋 **보류 과제는 [`TODO.md`](TODO.md) 에 모아 뒀다**(2026-08-21 신설). 이 파일의 세션 기록 속
> "미반영/보류" 항목이 흩어져 묻히는 걸 막으려는 것이다. **이미 조사 완결 후 '불가'로 판정된
> 항목**도 거기 있다 — 특히 **감정평가서 PDF(→ `TROUBLESHOOTING.md` §47)** 는 협회 뷰어가
> IE 플러그인 전용이라 우리 쪽에서 불가다. **같은 조사를 처음부터 다시 하지 말 것.**
...
> … 신규 사건번호도 함께 업데이트되는 것인지" → 이어서
> "취소 사건번호 예정 경매사건 등 전부 업데이트 되는것인지"

### 조사 결과 (전부 코드·데이터 실측) — 다음 세션이 다시 조사하지 말 것
**자동 워크플로는 3개뿐이고 사진·상세는 거기 없다.**

| 워크플로 | 주기 | 갱신 대상 |
|---|---|---|
| `collect-auction` | 매일 07:00 KST | `auction.json`(사건 목록) + `notices.json` |
| `collect-redevelop` | 매일 07:00 KST | `redevelop_seoul/polygons.json` |
| `collect-realprice` | 매월 1일 | `realprice_*.json` |

- **사진(`collect_auction_photos.js`)·상세(`collect_auction_detail.js`)는 워크플로에 없다 = 수동 실행만.**
- **사진은 기존 것을 다시 받지 않는다**: 대상 선정이 `!photosDb[cn]` — 사건번호 기준으로
  이미 있으면 건너뛴다(행 인덱스에 의존하지 않아 목록 순서가 바뀌어도 안전).
  `--force` 옵션이 **아예 없어** 강제 재수집이 구조적으로 불가능하다.
  - ⚠️ **이면의 한계(미해결)**: 이미 사진이 있는 사건에 법원이 사진을 **추가**하면
    "이미 있음"으로 건너뛰어 **새 사진을 영구히 못 받는다**(유찰 후 재감정 등).
- **`auction.json` 은 매일 전량 교체**(원자적 rename, 병합 아님). 가드: 0건 또는 직전의
  절반 미만이면 중단. → **취소·취하·매각완료 사건은 다음 날 목록에서 자연히 사라진다**
  (상태 분포에 취소/취하가 0건인 것으로 확인). 별도 취소처리 로직은 없다.
  - 부작용: 사건이 빠져도 사진 메타는 안 지워져 **stale 310건**이 남아 있다(미정리).
- **실제 데이터 창은 약 2주**: 현재 `auction.json` 매각기일 범위 2026.08.18~08.28.

### ⛔ 가장 큰 구멍 — 매각예정 물건이 한 번도 수집되지 않았다
법원 사이트는 화면이 둘이다: **PGJ151F00(진행, 매각기일 오늘~+2주)** / **PGJ157M00(매각예정,
오늘~+2개월)**. 수집기에 `--sched` 모드가 **이미 구현돼 있었는데** 워크플로가 플래그 없이
돌아서 `auction_sched.json` 이 **저장소에 존재조차 하지 않았고**, `land.html` 도 참조하지
않았다. 즉 **2주 밖 물건은 앱에 아예 없었다** — 경매는 미리 봐두는 게 중요한데 발굴이 불가능.

### 조치
1. **워크플로**: `collect-auction.yml` 에 `node tools/collect_auction.js --sched` 단계 추가.
   위 실행과 **순차**로 둔다(같은 사이트 동시 스크래핑은 IP 차단 위험).
2. **프론트**: 레이어 트리 `법원경매 > 매각예정 물건(~2개월)` 신설(`auctionSchedCluster`).
   진행(주황/빨강)과 구분되게 **남보라 `#7048e8`** 고정.
   - ⚠️ **팝업을 `auctionPopupHtml` 로 재사용하면 안 된다**: 그 팝업은 `data-i` 인덱스를
     `aucPopWire` 에서 **`auctionRows`(진행)** 로 되찾으므로 예정 물건에 쓰면 즐겨찾기·
     바로가기가 **엉뚱한 사건**에 걸린다. 그래서 예정은 전용 경량 팝업(`auctionSchedPopupHtml`)을
     쓰고 즐겨찾기·바로가기를 아예 넣지 않았다. 사진·상세도 예정 물건은 수집 대상이 아니다.
3. **당사자내역**은 §직전 커밋(`7ca0999`) 참고 — pgj15A 응답에서 수집하도록 고쳤다.

### 진행 중(이 세션에서 시작)
사용자 선택: **예정물건 살리기 + 재수집은 미보유분만**.
`tools/_chain_collect.sh` 로 **순차** 실행 중 — ① `--sched` 수집 완료 대기 → ② 사진 미보유
222건 → ③ 상세 미보유 154건(이번에 받는 건은 당사자내역까지 저장된다).
로그: `tools/sched_collect.log`, `tools/_chain_collect.log`. **자동 커밋·PC종료는 하지 않는다**(사람 확인 후 수동 커밋).
- 당사자는 사용자 판단으로 **전량 재수집(5시간) 안 함** → 앞으로 새로 수집되는 사건부터 쌓인다.
- `--sched` 첫 실행 시 PGJ157 화면이 3회 재시도 후 열렸다(문서화된 비결정적 로드, 정상 동작).

---

## 2026-08-21 (73) — Claude Code (성능 4단계: 청약공고 24시간 캐시 / 환율은 의도적으로 제외)

> 사용자: "환율은 실시간 변동이되고있고, 청약공고는 몇분사이에 바뀌지 않기는해
> 지금 청약공고는 얼마나 자주 요청하고있어?"

### 먼저 실측한 '현재 요청 빈도' (질문에 대한 답)
- 청약은 **페이지 로드마다 2회** — `getAPTLttotPblancDetail` + `getRemndrLttotPblancDetail`.
  `renderSubscriptions()` 가 로드 시 **무조건** 1회 호출된다(배지를 보든 안 보든).
- **한 탭 안에서는 재요청 없음** — `_subPromise`(메모리) + `_subRendered` 플래그로 막혀 있다.
  청약을 다시 부르는 `setInterval` 은 없다(기존 setInterval 2개는 호버 툴팁·하단 통계바용).
- 문제는 **저장되는 캐시가 없다**는 것 → 새로고침·새 탭이면 매번 2회. 하루 10번 들어오면 20회.

### 조치 (커밋 `1803216`) — localStorage 24시간 캐시
사용자 선택: **청약 24시간 / 환율은 그대로**(실시간 변동이라 캐시하면 틀린 값을 보여준다).
날씨·방문자수도 손대지 않았다.
- `mj_chungak_v1` = `{ t: 저장시각, rows: [...] }`, TTL 24h.
- **⚠️ 읽을 때 접수마감일 필터를 다시 적용한다(`chungakFresh`).** 캐시가 24시간 사는 동안
  접수가 끝난 공고가 생기므로, 캐시를 그대로 믿으면 **지난 공고가 남는 회귀**가 된다.
  원래 `push()` 의 `end >= todayStr` 와 같은 기준(`rcptEnd`)을 쓴다.
- 네트워크 실패 시 `chungakCacheRead(true)` 로 **기간 지난 캐시라도 사용**(빈 화면보다 낫고
  마감 필터는 유지). 실측 중 이 Edge Function 들이 동시 5개 호출 부하에서 **간헐적 500**을
  내는 것을 확인했는데(무료 티어 cold start, 기존 현상 — 직접 단건 호출하면 전부 200),
  캐시가 그 실패까지 완화한다.
- 저장 실패(용량초과·사생활모드)·깨진 JSON 은 try/catch 로 '캐시 없음' 취급.

### 검증
- 1회차 클린 로드: 청약 2회 호출 → 캐시 4건 기록, 경고 0
- 2회차(캐시 있음): **청약 호출 0건** / 환율 1건·날씨 1건 그대로 = 의도대로
- 마감된 공고를 섞은 캐시 → 마감분만 걸러지고 유효분만 반환
- 24시간 지난 캐시 → 무시하고 재요청(`null`)
- 깨진 JSON 캐시 → throw 없이 안전히 무시

### 성능 작업 최종 요약 (§70~73)
| 항목 | 이전 | 이후 |
|---|---|---|
| 초기 로딩(정비 폴리곤 gzip) | 3.49MB | **1.37MB** |
| 지도 살짝 이동(jbBuild) | 133~267ms | **0.3~0.7ms** |
| 지도 패닝(jbBuild) | 45~170ms | **3~20ms** |
| 경매 레이어 켤 때 | 상세 1.59MB 대기 | **대기 없음** |
| 재방문 시 청약 호출 | 2회(각 1.7초) | **0회** |

---

## 2026-08-21 (72) — Claude Code (성능 3단계: 경매 상세를 마커 렌더에서 분리)

> 사용자: "2번은 이해했어, 지금 개선해도 좋을것같아" (§70 남은 후보 2번 승인)

경매 레이어를 켜면 세 파일을 `Promise.all` 로 묶어 받았다 — `auction.json`(gzip 0.16MB) +
`auction_photos.json`(0.17MB) + **`auction_detail.json`(1.59MB, raw 9.01MB)**. 그래서 지도에
마커를 찍으려면 상세까지 다 받기를 기다렸다(합계 raw 14.4MB). 그런데 상세(기일내역·감정평가
요항표 등)는 **팝업/상세 패널을 열었을 때만** 쓰는 데이터다.

### 조치 (커밋 `9da7412`)
- 세 fetch 를 **같은 시점에 병렬로 시작**(총 바이트·도착시간 그대로)하되, 마커·목록 렌더를
  상세에 묶지 않는다. `loadAuctionDetail()` 이 따로 받고, 도착하면 `aucApplyDetail()` 이
  행에 붙인 뒤 열려 있는 상세 패널/목록을 다시 그려 '앱에서 열림(●)' 표시를 갱신한다.
- **행과 상세는 어느 쪽이 먼저 끝나도 붙는다** — 양쪽(`_auctionPromise` 완료 시,
  상세 도착 시)에서 `aucApplyDetail()` 을 호출한다. 순서에 의존하지 않는다.

### ⚠️ 여기서 반드시 막아야 했던 함정 — 로딩 중 클릭의 잘못된 법원 폴백
`aucLocalData(d, label)` 은 `d.detail` 로 "앱에서 열 수 있나"를 판정한다. 상세 도착 전에
'감정평가서'를 누르면 **데이터가 있는 사건인데도 "없음"으로 보고 법원 사이트를 열어버린다.**
→ `apdGoClick` / `aucPopGoClick` 이 상세가 없으면 `loadAuctionDetail()` 을 기다린 뒤 재실행한다.
(`.catch(() => ({}))` 로 실패해도 `{}` 로 resolve 하므로 무한 재귀는 없다.)

### 검증 — 로딩 구간을 인위적으로 만들어 측정
localhost 는 상세가 104ms 에 끝나 차이가 안 보인다. 그래서 `window.fetch` 를 감싸
**`auction_detail.json` 만 3~4초 지연**시켜 실제 로딩 구간을 만들고 측정했다.
- 목록 3,382행이 **52ms** 에 렌더 — 4초 지연된 상세를 기다리지 않았다(이전 구조라면 4초 대기)
- 세 요청 모두 `+1ms` 에 시작 — 병렬 유지, 상세 도착이 늦어지지 않음
- 로딩 중 '감정평가서' 클릭 → **`window.open` 호출 0건** (법원으로 새지 않고 대기)
- 상세 도착 후 → 버튼에 ● 자동 표시(사건내역·기일내역·감정평가서·매각물건명세서),
  여전히 `window.open` 0건 = 앱 안에서 처리됨
- 콘솔 에러 0

**검증 함정**: 처음 측정은 무효였다 — `localStorage(mj_layer_tree_v1)` 에 경매 레이어가
켜진 채로 저장돼 있어서, `fetch` 를 감싸기 **전에** 이미 상세가 로드돼 있었다(버튼에 ●가
처음부터 떠 있어 눈치챘다). 로딩 구간을 테스트할 때는 **`localStorage.clear()` + 새로고침**
으로 레이어가 꺼진 상태에서 시작할 것.

### 남은 후보
- §70 의 3번(푸터 위젯 Supabase 5개 호출, 각 0.4~1.7초) — 병렬이고 지도 렌더를 막지 않아
  체감 영향이 가장 작다. 값 캐싱(예: 15분)으로 재방문 시 호출을 없앨 수 있다. 미착수.

---

## 2026-08-21 (71) — Claude Code (성능 2단계: 정비구역 재빌드 스킵 + 폴리곤 부분 재사용)

> 사용자: "진행해줘" (§70 의 남은 후보 1번 = jbBuild 재빌드 구조 개선 승인)

`jbBuild()` 는 지도 `moveend` 마다(250ms 디바운스) 돌면서 `clearLayers()` 로 폴리곤을 전부
버리고 다시 만들었다. 두 단계로 고쳤다.

### 1단계 — 같은 그림이면 건너뛴다 (커밋 `8e34f77`)
작은 이동은 뷰포트에 10% 마진이 있어 **보이는 집합이 그대로**인 경우가 많은데도 전량
재생성했다. 렌더 결과가 같아질 조건이면 즉시 return.
- 시그니처 = 보이는 행 목록(뷰포트·`jbFilter`·`jbActiveStage` 가 전부 반영됨)
  + `showStageBadge`(줌 14 경계) + `jbSel.on`. 데이터는 `jbRows`/`jbPolys` **참조 비교**로 무효화.
- 실제 레이어 수도 함께 비교 — 다른 코드가 레이어를 비웠다면 스킵하지 않고 스스로 복구한다.
- `jbSel.set`(선택 목록)은 넣지 않는다: 선택 토글은 `jbSelStyleOne`/`jbSelApplyStyles` 로 기존
  레이어에 직접 스타일을 입히므로 스킵해도 이미 올바르다.
- **줌이 바뀌어도 재투영·smoothFactor 재단순화·툴팁 재배치는 Leaflet 이 알아서 한다** —
  집합이 같으면 줌 변경만으로 재빌드할 이유가 없다.
- 결과: **133~267ms → 0.3~0.7ms**

### 2단계 — 이미 그려진 폴리곤 재사용 (커밋 `0ba9c51`)
집합이 실제로 바뀔 때도 빠르게. 실측: 패닝 시 직전과 **79~98%(보통 94%) 겹친다.**
→ 벗어난 것만 제거, 새로 들어온 것만 생성(`_jbLive`: 행 인덱스 → 레이어).
남는 레이어는 지도에서 떼지 않으므로 재투영·permanent 툴팁 DOM 재생성이 없다.
- 결과: **45~170ms → 2.9~20.6ms** (패닝 8회 합계 1003ms → 57ms)

**⚠️ 여기서 가장 조심할 것 — 클릭 우선순위(z-order).** 새 폴리곤은 draw order 맨 위에
붙어서, 새로 들어온 **큰** 구역이 기존 **작은** 구역의 클릭을 가로챈다(사당4동 43만㎡가
사당동 305-35 4.13만㎡를 삼키는 기존 사례). 그래서 추가가 있었으면 전체를 **면적 내림차순
`bringToFront`** 해 순서를 되돌린다(마지막에 불린 = 가장 작은 것이 위). canvas 렌더러의
draw order 링크드리스트만 손대므로 재투영이 없다. **실측 역전 0건.**

**부분 재사용을 포기하는 조건**(전량 재생성으로 폴백):
- `showStageBadge` 변경(줌 14 경계) — 남은 레이어의 툴팁 종류까지 갈아야 한다
- `jbRows`/`jbPolys` 가 새로 로드됨 — 인덱스 의미가 달라진다

팝업 복원은 **팝업이 실제로 닫힌 경우(`!map._popup`)에만** 한다 — 재사용 시엔 안 닫히므로
그냥 다시 열면 깜빡임만 생긴다.

### 검증 (브라우저 실측)
- z-order 역전 0건(폴리곤 90개) · 줌 15↔13 툴팁 전환 정상(67 permanent ↔ 834 sticky)
- 먼 곳 이동 시 이전 구역 잔존 0건 · **누수 없음**(레이어 수 = DOM 툴팁 수: 46/46·55/55·62/62·71/71)
- 팝업: 최초 / 재사용된 레이어 / 새로 추가된 레이어 모두 정상 오픈(내용 확인)
- 선택 모드: 선택 시 분홍 → 작은 패닝 후 **같은 레이어 객체 재사용 확인** + 분홍 유지 →
  해제 시 원색 복귀. 먼 곳 새 레이어는 분홍 아님. 모드 해제 후 팝업 정상 복구.
- 콘솔 에러 0

### 검증할 때 빠지기 쉬운 함정 (내가 실제로 두 번 헛짚었다)
1. **`map.setView(...,zoom)` 은 애니메이션이라 직후 `map.getZoom()` 이 아직 옛 값**이다.
   그 상태로 `jbBuild()` 를 부르면 "줌이 바뀌었는데 스킵됐다"는 **오진**을 한다.
   → `{animate:false}` + 충분한 대기(1.2초, 250ms 디바운스보다 길게) 후에 확인할 것.
2. **팝업을 열면 autoPan 으로 지도가 움직여 moveend → 재빌드**가 돈다. 그 결과 방금
   잡아둔 레이어가 뷰포트를 벗어나 제거되고, 그 객체로 `openPopup()` 하면 안 열린다.
   "팝업이 깨졌다"고 오진하기 쉽다. → 팝업 테스트는 **한 번에 하나씩**, 이전 팝업을 닫고
   상태가 가라앉은 뒤에.
3. 선택 모드를 켠 채로 테스트가 중단되면 그 뒤 모든 팝업이 unbind 상태로 남아, 다음
   테스트에서 "팝업이 전부 사라졌다"로 보인다. → 새로고침 후 재측정.
