...
> ## ✅ 검증 완료 기준점: 태그 `stable-20260821`
>
> **2026-08-21, 사용자가 "모든 기능 요구에 대하여 전부 잘동작"으로 확인한 상태.**
> 이후 무언가 깨졌을 때 **맨손으로 원인을 추측하지 말고 이 태그와 비교부터 하라.**
> (해시 `bedb1d6` 이지만 **해시 대신 태그 이름을 쓸 것** — 위 🔴 항목처럼 해시는 죽을 수 있다.)
>
> ```bash
> git diff stable-20260821 --stat          # 그때 대비 무엇이 바뀌었나(파일 목록)
> git diff stable-20260821 -- land.html    # 특정 파일의 변경 내용
> git log --oneline stable-20260821..HEAD  # 그 뒤로 쌓인 커밋
> ```
>
> 되돌리기(필요한 만큼만 — 전체 reset 은 피할 것):
> ```bash
> git checkout stable-20260821 -- land.html   # 파일 하나만 그때 상태로
> git revert <문제커밋>                        # 특정 커밋만 취소(히스토리 보존)
> ```
>
> **이 태그에서 동작이 확인된 것**: 경매 사진 파이프라인 전체(수집→400px 축소→R2 업로드→
> 메타 JSON 배포, R2 40,198개 = 로컬 파일 수 일치) · 팝업 사진 클릭→라이트박스 확대·이전/다음 ·
> 라이트박스 프레임 크기조절 + 프레임에 맞춘 사진 확대·축소 · 사진 자체 확대 1~6배(버튼·휠·
> 더블클릭)와 확대 중 드래그 이동 · 확대 버튼 = 우하단 그립 바로 위 · 정비구역 폴리곤 이중
> 렌더링(“그림자”) 해소 · `tools/finish_auction_collect.ps1` R2 검증 단계 수정.
>
> ⚠️ **사진 바이너리는 git 에 없다**(`.gitignore`, R2 에만 있음). 이 태그로 코드를 되돌려도
> 사진은 영향받지 않는다 — 반대로 **코드를 되돌려도 사진 문제는 안 고쳐진다.**
...
> 새 세션은 그 항목부터 확인할 것.
>
> 🔒 **팝업을 건드릴 일이 있으면 [`LOCKED_POPUP_SPEC.md`](LOCKED_POPUP_SPEC.md) 가 정본이다.**
> 이 파일의 옛 세션 기록에는 나중에 되돌려진 접근법도 섞여 있다(예: 팝업 리사이즈 스트립 핸들,
> 그립 음수 bottom, 마커 자석 기능). **이 파일에서 팝업 코드를 가져오지 말 것** — LOCKED 파일에서.
>
> 🔴 **이 파일에 인용된 2026-08-20 이전 커밋 해시(약 100개)는 전부 무효다.** `git filter-repo` 로
> 히스토리를 재작성해 모든 해시가 바뀌었다. `git show` 가 `unknown revision` 을 내도
> **"그 수정은 안 됐다"고 판단하지 말 것** — 기록은 유효하고 해시만 죽었다. 사실 확인은
> 해시가 아니라 **코드를 직접 grep** 해서 하라. 대응표:
> `C:\Users\conoc\matjip_backup_before_filter_20260820\_filter_repo_maps\commit-map`
>
> ⛔ **철회된 기능**: 마커 자석 / 커스텀 커서 (세션 56 에서 전면 철회, 코드에 0건).
> 세션 55 의 "마커 자석기능 개선" 기록은 그 뒤 철회됐으므로 최종 상태가 아니다.
> **사용자 지시: 커스텀 커서는 처음부터 제안하지 말 것.**

---

## 2026-08-21 (70) — Claude Code (성능: 좌표 정밀도 정리로 로딩 2.19MB·지도 이동 렉 3배 개선)

> 사용자: "현재 matjip 이 전반적으로 느린듯해, 코드 최적화 가능한가?"

### 실측으로 찾은 병목 (추측으로 고치지 않았다)
1. **수집기가 좌표를 소수점 14~15자리로 저장**했다(예: `37.5427918959895` — 나노미터급).
   `redevelop_polygons.json` 은 좌표점이 **257,386개**라 이 낭비가 파일을 통째로 부풀렸다.
2. 그 파일은 **'정비사업 상세' 레이어가 켜져 있으면 페이지 열 때마다** 받는다. 레이어 상태가
   `localStorage(mj_layer_tree_v1)` 로 유지되므로 **재방문자는 매번 gzip 3.44MB 재다운로드 +
   raw 9.67MB JSON.parse**. (신규 방문자는 안 받는다 — 최초 방문은 `realprice_apt.json` 뿐)
3. **`jbBuild()` 가 400~530ms** 이고 지도 `moveend` 마다(250ms 디바운스) 돈다 → pan/zoom 할 때마다
   UI 가 0.5초 멈춤. **이게 "전반적으로 느림"의 주범.**

### 조치 (커밋 `4d70bf2`) — `tools/trim_coord_precision.mjs` 신설, 좌표 6자리 절삭
| 파일 | gzip | raw |
|---|---|---|
| `redevelop_polygons.json` | 3.44MB → **1.36MB (-60%)** | 9.67 → 5.66MB |
| `realprice_seoul_gg.json` | 0.32 → 0.24MB (-25%) | 1.75 → 1.60MB |
| `redevelop_seoul.json` | 0.14 → 0.11MB (-21%) | 0.81 → 0.76MB |

**부수 효과(실측)**: raw 가 줄어 파싱·투영 비용도 함께 줄어 **`jbBuild()` 400~530ms →
133~267ms (약 3배)**. 지도 이동 멈춤이 그만큼 짧아졌다.
**라이브 확인**: `redevelop_polygons.json` 전송 3.49MB → **1.37MB**
(CDN 이 옛 파일을 캐시하므로 확인할 때 `?cb=<임의값>` 을 붙여야 한다 — 안 붙이면 옛 크기가 나온다.
앱은 이미 `?v=` 를 붙여 fetch 하므로 사용자에게는 새 파일이 간다.)

**왜 6자리가 안전한가**: 위도 6자리 = 약 **0.11m**. 이 폴리곤은 이미 `smoothFactor:1` 로 그려
줌 17에서 **최대 약 1.2m** 오차를 허용한다 — 기존에 받아들인 오차보다 10배 정밀하다. 화면 차이 없음.

### ⚠️ 자동 갱신이 최적화를 되돌리는 문제도 같이 막았다
`collect-redevelop`(**매일 07:00 KST**)·`collect-realprice`(월간)가 이 파일들을 재생성·커밋한다.
그대로 두면 **다음 실행에서 좌표가 다시 15자리로 돌아가 최적화가 사라진다**(실제 위험이었음).
→ 두 워크플로의 수집 단계 **직후**에 trim 단계를 넣었다. CI 에서는 그 워크플로가 커밋하는
파일만 인자로 넘겨(`node tools/trim_coord_precision.mjs <파일들>`) 무관한 diff 를 만들지 않는다.
**수집기를 새로 만들거나 데이터 파일을 추가하면 이 단계도 같이 챙길 것.**

### 검증
절삭 후 구역 2,974 / 링 3,043 / 점 257,386 (**수량 동일**) · 잘못된 좌표 0 · 최대 소수자리 6 ·
realprice 9,030건 유지. 브라우저에서 폴리곤 정상 렌더(성수동 경계가 서울 bbox 내), 콘솔 에러 0.
스크립트 **멱등** 확인(재실행 시 추가 변화 0). `--check` 로 미리보기 가능.

### 남은 최적화 후보 (아직 안 함 — 기록만)
- **`jbBuild()` 가 여전히 133~267ms.** 원인은 폴리곤 생성이 아니다(300개 생성 = 1.1ms 실측,
  클러스터 add = 0.5ms). `clearLayers()` 후 **뷰포트 폴리곤을 매번 새로 만들어 투영**하는 구조 자체다.
  개선안: 이미 그려진 폴리곤을 재사용(뷰포트에 남아 있으면 유지)하고 새로 들어온 것만 추가.
  **검증된 코드를 건드리는 리팩터라 사용자 동의 후 진행할 것.**
- `auction_detail.json` gzip 1.59MB / raw 9.01MB — 경매 레이어 켤 때 `auction.json` +
  `auction_photos.json` 과 함께 **한 번에 14.4MB(raw)** 를 받는다. 상세는 팝업 열 때만 필요하므로
  사건별 지연 로딩(또는 분할)로 줄일 수 있다. 구조 변경이라 미착수.
- 배포에 불필요한 파일: `tools/.geocache.json`(2.46MB), `land.backup-20260815.html`(0.37MB).
  **페이지가 fetch 하지 않으므로 체감 속도와 무관** — 저장소/Pages 용량 위생 문제일 뿐. 문서가
  backup 파일을 복원 근거로 참조하므로 함부로 지우지 않았다.
- 푸터 위젯 Supabase Edge Function 5개(청약 2·환율·날씨·방문자수)가 **각 0.4~1.7초**.
  병렬이고 지도 렌더를 막지 않지만 연결을 5개 쓴다. 캐싱(예: 15분 TTL)으로 줄일 수 있다.

---

## 2026-08-21 (69) — Claude Code (사진 프레임↔이미지 연동·확대버튼 이동 + 정비 폴리곤 '그림자' 원인 규명)

> 사용자: "프레임 자체 크기조절시 사진도 자동으로 크기 조정되었으면 좋겠어, 그리고 …
> 사진 크기 조정하는 버튼 … 프레임창 크기 조정하는 버튼 그 한줄 위에 위치했으면해
> 또한 도시계획사업 … 폴리곤안에 그림자 효과가 있는듯해 … 폴리곤과 아웃라인만 보이게"

### 1. 완료 — 프레임을 키우면 사진도 커진다 (커밋 `daf9c2b`)
§68 에서 이미지에 `max-width/height:100%` 를 썼는데, 이건 **원본보다 크게는 절대 늘리지
않는다**. 경매 사진은 대부분 400px 로 축소돼 있어(§65) 창을 키워도 사진은 400px 그대로였다.
→ `width/height:100%` + **`object-fit:contain`** 으로 교체. 비율 유지하며 창에 맞춰 확대·축소.
transform 도 `-50%` 중심맞춤을 걷어내고 `translate(pan) scale(zoom)` 으로 단순화,
`lbClampPan()` 은 뷰 크기 기준으로 재계산(이미지 박스 = 뷰 박스가 되어 식이 간단해짐).
- 실측: 프레임 520 → 이미지 347 / 프레임 1100 → 643 (따라 변함). 줌 1.4배·팬 클램프 184px
  (=(920×1.4−920)/2 이론값 일치)·리셋 정상.

### 2. 완료 — 확대 버튼을 우하단 그립 위로 (커밋 `daf9c2b`)
`.apd-lb-zoom` 을 좌상단 → `right:6px; bottom:36px`. 창 크기조절 그립(`.ui-grip-corner`,
right/bottom:3px, 24px)의 **바로 위 한 줄**. 실측: `zoomIsAboveGrip` true, 우측 정렬 6px vs 3px.

### 3. 완료 — 폴리곤 '그림자'는 그림자가 아니라 **이중 렌더링**이었다 (커밋 `834b42c`)
**원인 규명(추측 금지 원칙대로 전부 실측)**:
- UPIS raster 타일(layer 96) PNG 를 직접 디코딩 → 색이 **정확히 3종**뿐:
  투명 / 외곽선 `0,112,255` / 채움 `192,225,252`. **그림자 픽셀 0개**(안티에일리어싱조차 없음).
- ArcGIS 심볼 정의(`/96?f=pjson`) → `esriSFSSolid` + 1.5px 외곽선, 라벨 `haloColor:null`.
  **서버 심볼에도 그림자·halo 없음.**
- 우리 `land.html`·`css/*.css` 전체에 폴리곤 대상 `drop-shadow`/`filter` **0건**,
  `.leaflet-overlay-pane` 의 computed filter 도 `none`.
- 즉 **'도시계획사업' 대분류를 켜면 UPIS raster 소분류(94~122)와 벡터 '정비사업 상세'
  (`jbPolyLayer`, 20% 채움+2px 외곽선)가 같이 켜져** 같은 구역을 두 번 그렸고, 두 경계
  데이터가 미세하게 달라 겹친 곳이 어둡고 외곽선이 이중 → "안쪽에 그림자"로 보였다.

**조치(사용자 선택: 벡터만 남기기)** — 클릭 팝업·구역 합산 선택 기능이 벡터에 붙어 있어
벡터를 남기는 쪽이 기능 손실이 없다.
- `jbDropDupRaster()`: 벡터가 켜지는 순간(`overlayadd`) '도시계획사업' 섹션의 raster 소분류를
  전부 해제. **`setTimeout 0`** 으로 대분류 일괄 토글(bigCb) 배치가 끝난 뒤 실행 —
  mid 순회 도중에 불려 순서에 따라 결과가 달라지는 걸 피한다.
- 벡터가 켜진 상태에서 그 raster 를 다시 켜려 하면 **막고 토스트로 이유를 알린다**
  (조용히 무시하면 "왜 안 켜지지?"가 된다).
- 소분류 체크박스에 `data-sec`(대분류명) 부여 — 중복 대상을 찾는 기준.

**실측 검증**: raster 96·112 켠 뒤 벡터 ON → 두 raster 자동 해제, 섹션 내 체크 0개,
지도에서 UPIS 레이어·타일 완전 소멸(`remainingUpis` 0 / `upisTileImgs` 0). 벡터 ON 상태에서
raster 재시도 → 차단 + 토스트 정상. 다른 섹션(지구단위계획 33)은 영향 없음(대조군).

**다음 AI 주의**:
- 정비 폴리곤은 **캔버스 렌더러**로 그려진다(`preferCanvas`). `.leaflet-overlay-pane path`
  를 세면 0 이 나오는데 **버그가 아니다** — 레이어는 `map._layers` 에 2,735개 정상 존재.
  SVG path 로 검증하려 들지 말 것.
- 브라우저 검증은 `_mockup_land.html`(auth-guard 제거 사본, `.gitignore` 커버) +
  `.claude/launch.json` 의 `matjip-static`(포트 8181) 조합으로 했다. 검증 후 사본은 삭제.
- raster 타일은 CORS 때문에 브라우저 canvas 로 픽셀을 못 읽는다(`getImageData` tainted).
  **curl 로 내려받아 Node 로 PNG 를 디코딩**해야 한다(위 3종 색 확인이 그 방법).

---

## 2026-08-21 (68) — Claude Code (사진 라이트박스를 크기조절 가능한 창으로 개편 + 이미지 별도 확대·이동)

> 사용자: "팝업에서 제공하는 경매 사진을 클릭하면 별도 프레임 창안 사진이미지가 있었으면해
> 그래서 그 프레임 도 크기조정할수있고 사진 이미지도 크기 조정가능하게 만들었으면 하는데"

§67 에서 붙인 라이트박스는 전체화면 백드롭에 사진이 바로 떠 있어 "창"이라 부를 게 없었다.
요청한 두 가지(프레임 크기조절 / 이미지 크기조절)는 서로 다른 조작이라 분리해서 구현했다(커밋 `80f2e3a`):

1. **프레임(`#apd-lb-frame`) 자체 크기조절**: 이미 있던 거리뷰 오버레이(`showStreetView`)와
   똑같은 공용 패턴 — `.ui-grip` 코너 그립 + `js/ui-resize.js` 의 `makeResizable()`.
   드래그로 창의 너비·높이를 조절한다(최소 280×220, 최대는 화면 여유). 프레임을 키우면
   맞춤 상태(줌 1배)의 이미지도 같은 비율로 커진다(`max-width/height:100%` 라서).
2. **이미지 자체 확대(줌)**: `+`/`−`/맞춤(⤢) 버튼 + 마우스 휠, 1~6배. 확대 중엔 이미지를
   드래그로 이동(pan) — 뷰 밖으로 과하게 안 나가게 `lbClampPan()` 이 잡아준다. 이미지
   더블클릭은 1↔2.2배 토글. **사진을 넘기면(이전/다음) 줌·이동 상태는 항상 초기화**된다
   (`lightboxRender()` 에서 `lbSetZoom(1)` 호출) — 안 그러면 확대해서 보다가 다음 사진으로
   넘어갔을 때 그 사진도 이상하게 확대·치우친 채로 뜬다.

**알아둘 것**: 경매 사진은 GitHub Pages 1GB 한도 대응으로 대부분 **400px 폭으로 축소**돼
있다(§65). 줌을 6배까지 열어놨지만 저해상도 사진은 3배 근처부터 흐려 보이는 게 정상이다
(원본 해상도가 그만큼이라 UI 문제가 아니다). 원본유지 2,364장은 상대적으로 선명하게 보인다.

**검증**: `node --check` 로 전체 스크립트 구문 확인. 줌/팬 로직(`lbSetZoom`/`lbClampPan`)만
따로 떼어내 가짜 DOM(`clientWidth`/`offsetWidth`/`classList` 만 흉내)으로 단위 테스트 —
줌 배율 계산(1→1.4→1.96), 큰 드래그의 팬 클램프(예상 최대치와 일치), 줌아웃 시 1배 바닥과
팬 리셋, 사진 전환 시 줌 리셋까지 전부 기대값과 일치 확인. 브라우저 라이브 검증은 관례대로
생략(§lean-commit-workflow) — 실제 화면은 사용자가 github.io 에서 확인.
...
### 6-1. 완료 — 기록 신뢰성 정비 (사용자 지시로 별도 진행)

> 사용자: "기존 기록들 확인하다가 예전에 잘못된 코드를 인식해서 문제 해결이 안되고 새로 코드짜고
> 수정하고 무한 반복하는 경우가 있어. 잘못되있던 코드 기록들은 다른 AI 인식도 못하게 만들어놔."

**원인 구조**: 문서가 시간순 기록이라 나중에 되돌려진 접근법이 여전히 "해결"로 남아 있었다.
그걸 읽은 AI 가 기각된 코드를 되살리고 같은 문제를 다시 "해결"하려 든다.

- **`LOCKED_POPUP_SPEC.md` 신설(정본)** — 현재 동작 코드를 verbatim 으로 담아 그대로 복원 가능하게.
  범위: 위치·크기·줌비례축소 / 드래그↔클릭 / 닫기·접기 버튼 / 스크롤바·그립. 실측 검증값과
  검증 스니펫, "하지 말 것" 목록 포함. `AGENTS.md` 읽는 순서 2번에 배치 + 우선순위 규칙 3개 신설.
- **기각 기록 무력화(코드를 지워 베끼지 못하게)**: §30(스트립 핸들+window click 차단) → "⛔ 기각됨",
  §30-2(그립 음수 bottom) → "⛔ 대체됨", §36 마커 자석 → "⛔ 철회됨"(실측 0건).
- **실제 버그 1건 발견·수정**: **CCTV 팝업 닫기 버튼이 2개**였다(실측 `[aria-label="닫기"]` 2개,
  둘 다 507,223). `openOn()` 이 `popupopen` 을 동기 발생시켜 `attachPopupControls` 가 먼저 붙이고,
  뒤이은 `setTimeout(…,0)` 이 존재 체크 없이 또 붙였다. 좌표가 같아 육안으로 안 보였다.
  → **다음에 X 위치를 옮기면 한 개만 옮겨져 2개로 보이고, §40 을 읽고 "또 버튼 위치 문제"로
  오진해 과거 8커밋 사이클이 재시작되는 시한폭탄**이었다. CCTV 두 분기의 닫기 생성 제거,
  컨트롤 추가를 `attachPopupControls` 한 곳으로 통일. 검증: 닫기1/접기1/그립1, 영상 슬롯 유지.
- **내 오진 정정**: `popup._adjustPan()` 이 안 통하는 이유를 "Leaflet 내부 조기 반환"이라고 4곳에
  잘못 적었다. 실제로는 **이 프로젝트가 `L.Popup.prototype._adjustPan` 을 `clampPopup` 으로
  통째로 교체**해 둔 것이었다(Leaflet 코드가 아니다). `clampPopup` 은 지도 경계만 보고
  `.ctl-row` 겹침은 보지 않는다. LOCKED §5-1 에 두 로직 공존 구조를 표로 정리.
- **문서가 현재 정답을 금지하던 지뢰 제거**: §2-1 "하지 말 것: ResizeObserver — 콜백 0회". 당시
  실측은 맞았고 **관찰 대상**이 달랐다(content 는 `--popup-max-h` 상한에 걸려 안 변함 / 래퍼는 변함).
- **커밋 해시 전량 무효 경고**: filter-repo 로 2026-08-20 이전 해시가 전부 죽었다(이 파일에 약 100개).
  "해시를 못 찾는다고 기록을 불신하거나 이미 된 수정을 되돌리지 말고 **코드를 grep 하라**" + 대응표 위치.
- **문서↔코드 값 불일치 4건 실측 정정**(전부 문서가 틀렸던 쪽 — 코드는 그대로):
  content `margin-right` 문서 0 / 실제 4.5px(스크롤바 우단에서 6px, 사용자 확인 상태) ·
  `.lp-body` 의 1000 은 CSS 선언이 아니라 Leaflet 기본값 ·
  §9 "지도 키 전부 공용 상수"는 거짓(KAKAO 3곳·네이버 2곳 중복, main.html 은 정적 script src 라
  구조적으로 공용화 불가) · §37 에 `padding-top:28px` 누락.

### 6-3. 완료 — 경매 팝업 기능 추가 + 팝업이 왼쪽 패널도 피하게 (2026-08-21)

> 사용자: "팝업에는 즐겨찾기 기능이 없고 … 아이콘으로 클릭한 경매가 좌측 패널 맨위에 나오도록
> … 팝업에 바로가기 버튼이 있으면 좋겠어 사건내역, 기일내역 감정평가 등"

**커밋 `511ca87`(기능) → `409696c`(패널 회피)**. 확정 코드는
[`LOCKED_POPUP_SPEC.md`](LOCKED_POPUP_SPEC.md) 가 정본이다(§2 BLOCK-D, §6, §6-9).

- **팝업 즐겨찾기(`.auc-pop-fav`)**: 즐겨찾기가 목록 행(`.ap-star`)에만 있어 지도 아이콘 클릭
  흐름에서는 못 썼다. 팝업 헤더에 별표를 넣고 **목록과 같은 저장소(`land_auction_fav`)를 공유**.
  토글 시 `auctionRenderList()` 를 불러 목록 별표·즐겨찾기 필터까지 함께 갱신한다.
- **팝업 바로가기 6종(`.auc-pop-go`)**: 상세 패널의 `AUC_GO_LABELS` 를 재사용. 동작도 상세
  패널과 동일한 두 갈래 — 사건내역·기일내역(+`detail` 있음)은 상세 패널을 열어 해당 섹션으로,
  문서류(감정평가서 등)는 사건번호 복사 + 법원 사건검색 새 창(문서 딥링크는 구조적 불가, §6-12).
- **클릭한 물건을 목록 맨 위로(`auctionPinToTop`)**: 3,382건 중 어느 행인지 찾을 수 없던 문제.
  `.pinned` 강조 + "지도 선택" 배지, 목록 `scrollTop=0`, 상세 패널이 열려 있었으면 목록으로 복귀.
  - **인덱스가 아니라 즐겨찾기 키(`cn|no|addr`)로 고정**한다 — 검색·즐겨찾기 필터로 목록이
    걸러져도 같은 물건을 가리켜야 한다.
  - **필터에 걸려 목록에 없으면 핀을 무시**한다 — 사용자가 걸어둔 필터를 임의로 풀지 않는다.
- **배선 방식(중요)**: 팝업 내용은 문자열로 만들어지고 Leaflet 이 `disableClickPropagation` 으로
  전파를 끊으므로 **document 위임이 통하지 않는다**. `popupopen` 에서 `aucPopWire()` 로 직접
  건다(모든 팝업 오픈 경로 커버). 식별자는 `data-favkey` / `data-i` 로 실어 보낸다.
- **팝업이 왼쪽 패널도 피하게** (`nudgePopupBelowControls` → **`nudgePopupIntoSafeArea`** 개명):
  `.left-panels` 는 `z-index:2000` 으로 팝업(1200)보다 위라 마커가 패널 뒤에 있으면 버튼을
  쓸 수 없었다. 상단 컨트롤(dy)과 왼쪽 패널(dx)을 함께 계산해 한 번의 `panBy` 로 처리.
  - 패널 폭을 상수로 박지 않는다 — 사용자가 200~640px 로 조절하고 접을 수도 있어 매번 실측.
  - **구조적 한계**: 팝업 폭 > (지도 폭 − 패널 폭) 이면 자리가 없다. 그때도 "전부 아니면 전무"로
    포기하지 않고 `dx = min(need, room)` 으로 **여유만큼은 민다**. 실측(지도 546/패널 340):
    팝업 493px → 겹침 299→295px(자리 부족, 버그 아님) / 팝업 177px → **겹침 0px**.
    즉 데스크톱 폭에서는 완전히 회피된다. 상세·금지사항은 LOCKED §6-9.

### 6-4. 완료 — 감정평가서·매각물건명세서를 앱 안에서 보게 (2026-08-21) / 문건·송달은 조사 대기

> 사용자: "법원기록을 먼저 다운해놔서 해당 내용을 matjip에서 볼수있으면 … 바로가기 버튼 눌러서
> 확인하면 대법원 홈페이지 가서 사건번호를 수기로 입력해야 확인가능하게 나오거든"

**핵심: 데이터는 이미 있었다.** `collect_auction_detail.js` 가 법원 응답에서 뽑아
`auction_detail.json` 에 저장하고 상세 패널이 렌더까지 하고 있었는데, 바로가기 매핑에
`사건내역`·`기일내역` 둘만 등록돼 있고 나머지 섹션엔 `id` 가 없어 전부 법원 링크로 폴백됐다.
**법원 접속 없이 프론트 수정만으로 해결.** 상세 표·필드 매핑은 `TROUBLESHOOTING.md` §46.

- 섹션 id 부여: `apd-sec-objct` / `apd-sec-notsugt` / `apd-sec-evlt` / `apd-sec-ndstrc`
- `APD_LOCAL_GO` 를 **라벨→섹션 후보 배열**로 변경, `감정평가서`(→`evlt`) ·
  `매각물건명세서`(→`objct`, 없으면 `ndstrc`) 추가
- **`d.detail` 존재만으로 판정하지 않는다**: `evlt` 가 빈 사건이 **11건** 있다.
  `aucLocalData(d, label)` 로 필드별 실제 데이터 유무를 본다 → 없으면 자동으로 법원 링크.
  실측 검증: 그 11건에서 `has-local` 미부착·툴팁 변경·법원 폴백 정상.
- 앱에서 열리는 버튼은 `.has-local`(테두리 강조 + `●`)로 구분 — 어느 게 법원으로 나가는지 미리 보인다.
- 팝업·상세 패널 양쪽 동일 판정(`aucPopGoClick` 도 `aucLocalData` 사용).
- **실측 검증**: 6개 버튼 중 앱내 4(사건내역·기일내역·감정평가서·매각물건명세서) / 법원 2(문건·송달, 현황조사서).
  감정평가서·매각물건명세서 클릭 → 해당 섹션으로 스크롤, **새 창 안 열림**. 문건/송달 → 법원 정상 폴백.

**보유 현황**: 감정평가 요항표 `evlt` **2,590/2,601** · 물건명세 `objct` 2,601 · 고지사항
`ndstrc` 대부분 · 제시외 `notsugt` 255 · **문건/송달 0 · 현황조사서 0**

### 6-2. 완료 — 수집 뒤처리 무인 자동화 (2026-08-21 00:53 가동)

사용자 요청: "수집 완료하면 커밋 푸시 배포까지 다하고 컴퓨터 자동종료".

- **`tools/finish_auction_collect.ps1` 신설** — 수집기 종료 대기 → 축소 검증 → R2 업로드 →
  R2 직접 LIST 대조 → `auction_photos.json` 커밋·push → **전 단계 성공 시에만** 컴퓨터 종료.
  상세·함정은 `TROUBLESHOOTING.md` §45.
- **가동 상태**: PID 43300 으로 분리 실행(세션과 무관하게 생존). 수집기 PID 32028 감시 중.
  로그 `tools/finish_auction_collect.log`(gitignored).
- **ETA 실측(2026-08-21 00:34 기준)**: 수집 583/1941, 분당 3.68건 → 수집 완료 약 06:45,
  축소 ~07:15, 업로드 ~08:00. 즉 **아침 8시경 전부 끝나고 PC 가 꺼져 있을 예정.**
- **취소 방법**: 종료 유예 120초 안에 `shutdown /a`. 자동화 자체를 멈추려면 PID 43300 종료.
- **실패 시 동작**: 컴퓨터를 끄지 않고 그 자리에서 멈춘다. 아침에
  `tools/finish_auction_collect.log` 를 보고 수동으로 이어서 하면 된다
  (`node tools/upload_r2.mjs` → `git add auction_photos.json` → commit → push).
- 수집 중에는 사진이 원본 크기로 쌓여 용량이 크게 늘어난다(298MB → 1.4GB 이상). 정상이며
  축소는 수집기가 끝날 때 한 번에 돈다. 축소 후 약 740MB 예상(파일 약 40,000개).
...
- **✅ 현재 정상 확인 상태 (2026-08-20)**:
  - 건물 팝업: 닫기(X) / 접기(−) 같은 줄, 스크롤바 위치 정상
  - CCTV 팝업: 닫기(X) / 접기(−) 같은 줄, 영상 재생 정상
  - 정비·실거래 등 기타 팝업: 동일 구조 적용, 모두 정상
...
| # | 데이터 소스 | 상태 | 이유 |
|---|-----------|------|------|
| 1 | 국토교통부 ITS CCTV (15040466) | ✅ 기존 완료 | 고속도로·국도만 (서울 285건) |
| 2 | 서울시 CCTV (TOPIS, OA-20477) | ❌ 서비스 종료 | data.seoul.go.kr에서 해당 데이터셋 폐쇄 |
| 3 | **전국 CCTV 표준데이터 (15155042)** | ✅ **이번에 완료** | data.go.kr OpenAPI, 자동승인, 전국 377,278건 중 서울 54,238건 |
| 4 | 지자체별 CCTV (부산/경기 등) | ❌ 불필요 | 표준데이터에 전국 지자체 포함 |
| 5 | 전국 CCTV CSV (15013094) | ❌ 프로그래밍 불가 | CSV 다운로드에 CSRF 토큰 필요 |
| 6 | V-World 국토교통부 교통CCTV | 미시도 | 표준데이터로 충분 |

### 2. collect_cctv.js 리팩토링 (커밋 `f813dd26`)
- 죽은 TOPIS 코드(OA-20477) 전부 삭제
- data.go.kr OpenAPI `https://apis.data.go.kr/1741000/cctv_info/info` 수집기 추가
- keys.env의 DGK 키 재사용 (별도 CCTV 키 불필요, 같은 data.go.kr 계정)
- ITS 코드 유지 (고속도로·국도 실시간 영상)
- 전국 377,278건 전체 수집 → 서울 좌표 필터링 → dedup
- 필드 매핑: WGS84_LAT/LOT, INSTL_PRPS_SE_NM(목적), MNG_INST_NM(관리기관), MNG_INST_TELNO(전화번호), CAM_CNTOM(카메라대수), SHT_ANGLE_INFO(촬영방향), KPNG_DAY_CNT(보관일수), INSTL_YM(설치연월), LCTN_ROAD_NM_ADDR(도로명주소)

### 3. land.html CCTV 팝업 분기 (커밋 `f813dd26`)
- `openCctvPopup(it)`: `it.url` 있으면 ITS 영상 팝업(기존), 없으면 표준데이터 정보 팝업(신규)
- 표준데이터 팝업: 목적·관리기관·전화번호·카메라대수·촬영방향·보관일수·설치연월·주소 표시
- CSS `.cctv-info-row` / `.cctv-info-k` / `.cctv-info-v` 스타일 미추가 (다음에 보강 필요)

### 4. cctv_static.json 결과
- **ITS 285건 + 표준데이터 54,238건 = 총 54,523건** (기존 278건 대비 196배)
- 목적별: 생활방범 37,741 / 교통단속 5,516 / 다목적 3,193 / 어린이보호 2,946 / 재난재해 1,325 / 쓰레기단속 1,065 / 시설물관리 726 / 차량방범 353
- ⚠️ 서울 외 경기도 등 14,762건 포함 (좌표 필터가 광명시 등 서울 인접 지역 포함) — address 기준 재필터링 필요

### 5. 미해결 사항
- **서울 외 데이터 필터링**: 좌표 필터가 느슨해서 광명시 등 포함. `collect_cctv.js`에서 address.startsWith('서울') 필터 추가 필요
- **CSS 보강**: 표준데이터 팝업의 `.cctv-info-row` 스타일 미정의 (현재 기본 텍스트로 표시)
- **배포 미진행**: 커밋만 완료, push/배포 대기
- **마커 300건 제한**: 54K건 중 뷰포트内马커만 표시하도록 수정 필요

### ▶ 이어서 할 일
1. **서울 외 필터**: `collect_cctv.js`에 address 기준 서울 필터 추가 + 재수행
2. **CSS 보강**: 표준데이터 팝업 스타일 정의
3. **배포**: land.html 변경분 반영 확인 (빌드 태그 갱신 필요 여부 확인)
4. **CCTV 렌더 최적화**: 54K건 중 뷰포트 내만 표시, 마커 수 제한 로직 강화
...
## 2026-08-16 (56) — opencode (커스텀 커서 자석 재구현 → 철회, 빌드 태그 갱신 관례 확립)

> 사용자: "공인중개사 아이콘이 마우스 커서로부터 도망간다" 재신고 → 배포본 실측 결과 **옛 코드 캐시**(배포본에 마커 이동 코드 없음)였지만, 사용자가 원한 게 "아이콘 고정 + 커서가 아이콘으로 끌려감"임을 확인.

### 진행 과정 (커밋 순서 — 커밋·push는 매번 사용자 동의 후)
1. `661eb7fa` 커스텀 커서 자석: 진짜 커서 숨김(`cursor:none`) + `#mag-cursor` 오버레이가 마커 45px 반경 진입 시 rAF lerp로 마커 중심으로 당겨짐. 아이콘은 고정(좌표 불변). 배포 실측: 자석 동작 확인.
2. `dc8c88aa` 커스텀 커서를 👆 이모지로 → 사용자: "원래 커서 손모양으로 해줘" → Wikimedia CC0 손모양 SVG로 교체(그러나 **세로 뒤집기 누락으로 손가락이 아래를 향함** — 사용자 불만).
3. `ee44547e` **사용자 지시로 자석 전체 철회**: "그냥 지도에서 마우스 커서 이동할때 그 아이콘 그대로 사용하면 되는데 왜그러는거야" → `#map.mag-cursor-hidden` CSS, magMarks/magCur/magTick/magHide/magnetize 블록, 호출 4곳(3247/5273/5309/5324 → 실측 시 라인) 전부 삭제. **진짜 브라우저 손모양 커서 그대로 복귀.**

### 교훈 (WHY)
- **커서 모양을 오버레이로 재현하려 하지 말 것.** 브라우저 기본 손모양 커서는 OS가 그리므로 HTML 요소로 100% 복제 불가. 사용자가 원하는 건 "원래 커서 그대로"일 때가 많다 → 커스텀 커서는 처음부터 제안하지 말 것.
- **빌드 태그 갱신 누락 적발**: land.html 푸터 `build 2026-08-04-b36`이 8/4 이후 계속 안 올랐다(자석·보안 등 커밋 여럿 있었음). 사용자: "아직도 b36이네? 잘 저장하고 백업하고 기록하는것 맞아?" → **이번에 `build 2026-08-16-b37`로 갱신.**

### ▶ 이어서 할 일
1. **배포마다 빌드 태그 갱신을 잊지 말 것** (land.html:730 주석 참고). 커밋 내용이 land.html 푸터에 반영될 변경이면 반드시 `build YYYY-MM-DD-bNN` (NN = 이전+1) 올리고 커밋한다. 이 HANDOFF 항목이 관례 기록.
2. (없음 — 자석 관련 코드는 전부 제거됨. 배포본에서 `mag-cursor`/`magnetize` 문자열이 없음을 확인하는 것으로 종결.)

---

## 2026-08-16 (55) — opencode (보안 감사: 취약점 4건 발견·기록만, 수정 미시작 — 순서·동의 대기)
...
### 결론 (근거 포함, 다음 AI가 같은 오해 반복하지 않도록)
- **"공공데이터라 출처 표기 생략 가능"은 오해**. 서울 열린데이터광장 이용약관(data.seoul.go.kr/etc/accessTerms.do): "저작자 및 출처 표시 조건으로 자유이용 허락"이 원칙. 공공데이터포털 데이터 상세 "이용허락범위"도 서울시·국토부 데이터 대부분 **공공누리 제1유형(출처표시)**. 공공데이터포털 가이드에도 "공공데이터라 출처 생략해도 된다고 판단"을 실수로 명시.
- 다만 **표기 형식은 자유**: 저작권법 제37조 "이용 상황에 따라 합리적인 방법" → 화면 항시 배지(네이버·카카오 SDK 약관)와 달리, **약관 한 문단 + 지도 attribution**이면 충족. 개별 항목 반복 표기 불필요.
- **네이버·카카오는 별개**: 지도 SDK 약관이 로고/© 표기 + 서비스명 노출 강제 → 화면 attribution 유지 필요.

### 적용 (커밋 `ea04a80e`, push 완료)
- `terms.html` 저작권 정책: 기관 나열 "VWorld, 국토교통부, OpenStreetMap, 네이버, 카카오 등" + "각 기관의 이용허락 조건(출처표시 등)에 따라 활용" 통합 선언으로 정리.
- 화면 표기는 기존 유지: land.html attribution(© V-World, © OpenStreetMap, © 서울도시공간포털), 국세청 출처 문구, main/detail attribution.

### ▶ 이어서 할 일
1. (없음 — 이 세션 결론은 위 기록으로 충분. 배포는 GitHub Actions 자동 반영되므로 확인만 하면 됨.)
2. 사용자 실사용 시 terms.html 렌더 확인.
...
### 적용 (land.html)
- **실거래 `uPriceShort`(1520)**: 상호명(단지명)이 항상 먼저 — `n + ' · ' + 가격`. 기존 주소가 앞에 나오던 문제 해소. 툴팁 경로 3곳(1626/1853 등)이 자동 반영.
- **EV `evBuild`(2113)**: `bindTooltip(esc(d.name))` — 충전소명.
- **경매 `auctionBuild`(2197)**: `bindTooltip(esc(d.addr))` — 경매는 상호명이 없어 **주소가 식별자**(WHY 주석).
- **CCTV 마커(3193)**: `bindTooltip(esc(it.name))` + **`magnetize(m)`** — 200개 제한이라 자석 포함. 클러스터(실거래/EV/경매)는 성능상 자석 제외.
- 자석 헬퍼(~1039)는 이전 세션에서 이미 존재(magnetize/magMarks, mousemove 45px 당김).

### 검증 (서버 8798 + Chrome 9223 CDP, probe_tips2/3/dbg/final)
- **실거래**: "e편한세상헤이리 · 3.4억", "산호2 · 2.4억", "시흥센트럴헤센 · 4.1억" — 단지명·가격 ✓
- **경매**: "경기도 시흥시 정왕동 2028-11"(지번), "거북섬타워 6층609호" — 주소(식별자) ✓ (경매 툴팁 = esc(d.addr) 실측 확인, `_idx` 플래그)
- **EV**: "목동신시가지아파트8단지", "서울영등포 코레일유통" — 충전소명 ✓ (아이콘 `#2f9e44` 구분)
- **CCTV**: "[수도권제1순환선] 성남", "[제2경인선] 광명" — CCTV 이름 ✓ (81개 마커)
- 마커 카운트: auc 47 / ev 28 / rp 10 / cctv 81 (fresh 로드). JS 오류 0건.
- **함정 2건 → TROUBLESHOOTING §35(블록 스코프)·§36(localStorage 레이어 복원)** — probe가 레이어 상태를 오염시킴(`mj_layer_tree_v1`), 검증 전 초기화 필요.
...
> 원인 규명(실측 분포 분석) → 임계값 100으로 수정·검증 완료 → 사용자 "정상이면 그냥 푸시해줘" → 커밋·push.

### 원인 (실측 근거)
- `usableRings()`의 `MIN_RING_M2 = 1000`이 진짜 가로주택(모아) 폴리곤을 통째로 걸렀다.
  - 전체 3,032링 실측 분포: p5=579.6㎡, p10=1,504㎡. **100~999㎡ 링 193개가 전부 진짜 가로주택**.
  - 반포동 717-16: area 4,512㎡, 링 333㎡ / 715-19: 7,600㎡/232㎡. 대표좌표·폴리곤 중심 0m 일치 → 진짜 구역.
  - 옛 주석("5% 87㎡/10% 6,590㎡ → 1,000 안전")은 **옛 데이터 기준**으로 틀렸음.

### 변경 (land.html)
- `const MIN_RING_M2 = 100;` (2734행) + 주석을 2026-08-16 실측 근거로 교체.
- 효과: 폴리곤 2,763→2,948개, 원 200→15개. 서초구 모아 42곳 전부 폴리곤.
- 남는 원 15개는 폴리곤이 대지 1필지 조각만 저장된 케이스(용문동 42-3: area 4,495㎡/링 14㎡) — 재수집 필요.

### 검증 (probe118·120, syncheck ALL OK, 서빙 사본 동기화)
- probe118: 717-16 `targetAsPolygon:true`, 폴리곤 55개/원 0개, JS 오류 0건.
- probe120: 팝업 진행현황 18단계 정상(준공·입주 "예정" 포함), 대지면적 333㎡ 표시.
- **함정 발견**: 테스트 site 폴더에 `redevelop_polygons.json` 없으면 `jbPolys={}` → 전 구역 원 폴백. 데이터 파일 서빙 확인 필수.
- TROUBLESHOOTING §32 기록.

### 커밋·배포 상태
- **이번 커밋·push 진행** (사용자 "정상이면 그냥 푸시해줘"). 변경: land.html + HANDOFF 47 + TROUBLESHOOTING 32.
- 배포는 GitHub Actions 자동 빌드 → 배포본에서 `MIN_RING_M2 = 100` + 717-16 폴리곤 렌더 실측 확인 필요.

### ▶ 이어서 할 일
1. push 후 배포 반영 확인: 배포본 land.html에 `MIN_RING_M2 = 100`, 반포동 717-16 폴리곤 렌더, 원 15개 감소.
2. (후속 후보) 남은 원 15곳 폴리곤 재수집(urban WMS UQ120) + 팝업 상세(사업개요·건축규모·공급규모) 보강.
...
## 2026-08-16 (46) — opencode ("실거래 거래 표에 마커에 가격도 나오게" — 아파트 실거래 마커에 가격 라벨 + 푸터 사업자등록번호)

> 사용자 요청: 실거래 레이어 마커에 가격 표시. 아울러 푸터에 사업자등록번호 추가.
> **마커 30,515개(거래 행) → 12,459개(단지별 최신 거래 1행)로 줄였다** — 같은 단지(같은 좌표)에
> 수십 개가 겹쳐 라벨이 쓸모없던 문제를 제거. 팝업은 rpGroup()으로 그 단지 전체 거래 표를
> 여전히 보여주므로 정보 손실 없음.

### 변경 (land.html, main.html)
1. **마커를 divIcon + 가격 라벨로** (`rpMarkerIcon` ~1624): 색 점(건축년도/등락, 기존 ageColor/rpMoveColor)
   + 가격 텍스트(`uPriceShort`). `rpBuild`(~1632)가 거래 행 대신 **단지(name|구|동)별 최신 거래 행 1개만**
   마커로 생성 — 필터는 행 단위로 먼저 걸고 남은 행 중 최신 1개를 대표로 선택.
2. **단위 토글 시 라벨 갱신** (`rpRefresh` ~1738): `uUnit`(총액↔평단가)이 라벨 텍스트를 바꾸므로
   `m._rpIcon` 마커는 `m.setIcon(rpMarkerIcon(m._rp))`로 아이콘만 교체 — 클러스터 통째 재생성
   (예전 33,810개 circleMarker 재생성, 2026-08-14 코드리뷰)를 피한다. divIcon html은 문자열일 뿐이라
   DOM은 마커가 지도에 나타날 때 만들어져 circleMarker 재생성만큼 비싸지 않다.
3. **푸터**: land.html/main.html 하단 `호룡리싱앤리얼리티 | 사업자등록번호 : 369-37-00360 | 오재호
   010.4353.6501 · conoc@naver.com`(land ~700, main ~174).
4. 주석 정리: openRpApt(~5520)가 "거래 행마다 마커"라고 적힌 옛 주석을 단지 1개로 수정.

### 검증 (probe114·115·116·117, syncheck ALL OK, 서빙 사본 동기화)
- probe114: `layerCount=12,459`(= 단지 수), `rawRowCount=30,515`, 첫 마커 html에 `9.3억` + 색 `#94d82d`
  라벨 포함. probe116: 지도 줌 13에서 마커에 "6.8억"/"3.6억" 등 가격 라벨 렌더, 클러스터 숫자와 공존.
- probe117: `u-price` 칩(평단가) 클릭 → 라벨 "6.8억"→"1,135만" 등으로 갱신 확인.
- JS 오류 0건. 스크린샷 `shot_marker_price.png`(모델이 이미지 미지원이라 텍스트 검증으로 대체).
- **주의**: 실거래 마커 개수 표시(있다면)가 거래 행→단지 수로 바뀜. 필터 동작은 동일(행 단위 필터 후 대표 선택).

### 커밋·배포 상태
- 직전 `ffb859c4`(45) push 완료·배포 반영 확인.
- **이번 커밋·push·배포 대기** — 사용자 "푸시해줘" 수신, 커밋 직전.

### ▶ 이어서 할 일
1. 커밋·push·배포 반영 확인(`마커에 가격` + 사업자등록번호 포함 검사).
2. 사용자 실사용 확인 후 후속 개선(매매/전세/월세 구분, 마커 단지 표시 UI 등) 판단.
