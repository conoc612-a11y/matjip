...
## ⓿ 무언가를 붙이려 할 때 — **이 순서로 본다** (1차 관문)

**data.go.kr 을 검색하거나 경로를 추측하기 전에 이 문서를 먼저 훑는다.** 아래 6단계를 위에서부터
확인하면, 대부분 ③ 이전에 답이 나온다.

| 단계 | 볼 곳 | 무엇을 확인하나 |
|---|---|---|
| ① | **§6 API별 블록** | **이미 연결돼 있나?** 있으면 호출 형태·응답 경로·함정을 그대로 복사한다. 새로 붙일 필요 없음 |
| ② | **§4 교차검증 매트릭스** | 원하는 **값**을 이미 다른 출처가 주고 있나? 주면 새 API 대신 그걸 쓰거나 교차검증에 넣는다 |
| ③ | **§2 활용/미활용** | **승인은 됐는데 안 쓰는 것**에 있나? 있으면 **신청이 불필요**하고 바로 구현으로 간다 |
| ④ | **§7-1 확인된 base URL** | 경로가 이미 확인돼 있나? 있으면 **절대 추측하지 말고** 그 값을 쓴다 |
| ⑤ | **§5 범주별 인덱스** | 같은 **범주**를 주는 다른 출처가 있나? 합치면 커버리지가 올라간다 |
| ⑥ | 여기까지 없으면 | data.go.kr 검색 → 상세페이지 `요청주소` 읽기(**§7-0**, 추측 금지) → 그래도 없으면 **공공데이터 외**(카카오·네이버 등 민간 API, ToS 확인) 검토 |

⚠️ **⑥으로 바로 뛰지 말 것.** 2026-08-22 실측 낭비: 기상 CCTV·주차장 API 경로를 6개 추측해 전부
`NO_OPENAPI_SERVICE` 였다. ①~④를 먼저 봤으면 "미구현이고 경로 미확인"임을 알고 곧바로
상세페이지를 읽으러 갔을 것이다.

⚠️ **공공데이터에 없는 것도 있다.** 그때는 민간 API 를 검토하되 **ToS 를 먼저 확인**한다 —
카카오 장소는 저장·재배포 제약이 있어 화면 표시만 가능하고(§6-11), 같은 상호를 저장해도 되는 것은
소상공인 상가정보다. **"데이터가 있다"와 "우리가 쓸 수 있다"는 다르다.**

📄 승인 목록 전체(90건)는 `docs/API_CATALOG_personal.md` — 사용자 개인 자료다. ③ 단계에서
"승인됐는데 §2 요약에 안 보인다" 싶으면 그 파일의 목록을 확인한다(목록은 사실이므로 참조해도 된다.
**스키마는 없다** — 호출해 본 적 없는 API 의 스키마를 그 파일에서 찾지 말 것).
...
## 2. 공공데이터포털 90건 — 무엇을 쓰고 무엇을 못 쓰는가

계정(conoc) 활용신청 **90건**(승인 87 · 신청중 3, 2026-08-22 실측). matjip 이 실제로 쓰는 것은
**17종**이고 **73종은 승인만 받아 두고 안 쓴다.**

📄 **전체 90건 목록은 `docs/API_CATALOG_personal.md`** (사용자 개인 자료 — AI 는 참조하지 않는다).
여기에는 **실측한 것만** 적는다. 호출해 본 적 없는 API 의 스키마를 적으면 AI 가 그걸 믿고
검증을 건너뛴다 — 틀린 스키마는 없는 것보다 나쁘다.
...
### 2-2. 🔒 승인만 받고 미활용 (73종) — 범주별 요약 (전체는 개인자료 참조)
...
### 7-0. ⛔ 엔드포인트 경로를 **추측하지 말 것** (가장 자주 낭비하는 지점)

2026-08-22 실측 낭비: 기상청 CCTV·주차장 API 경로를 6개 추측해 전부 `NO_OPENAPI_SERVICE` 였다.
서비스명·오퍼레이션명은 **규칙이 없다.** 같은 기관, 같은 계열에서도 제각각이다:

```
건축물대장   1613000/BldRgstHubService/getBrTitleInfo      ← getBr*
건축인허가   1613000/ArchPmsHubService/getApBasisOulnInfo  ← getAp*  (같은 1613000 인데 다르다)
실거래       1613000/RTMSDataSvcNrgTrade/getRTMSDataSvcNrgTrade  ← 서비스명 반복
소상공인     B553077/api/open/sdsc2/storeListInPnu        ← get 접두사 없음
LH 임대      B552555/lhLeaseInfo1/lhLeaseInfo1            ← 서비스명 반복 + ServiceKey 대문자 S
온비드 목록  B010003/OnbidRlstListSrvc2/getRlstCltrList2
```

**반드시 data.go.kr 상세페이지의 `요청주소`/`Base URL` 을 읽어서 쓸 것.** 방법(실측으로 검증된 것):

```js
// data.go.kr/data/{ID}/openapi.do 로 이동한 뒤 브라우저 콘솔에서
(() => { const t = document.body.innerText.replace(/\s+/g, ' ');
  const i = t.indexOf('상세기능'); return t.slice(i, i + 900); })()
// → '요청주소 https://apis.data.go.kr/...' 와 '요청변수' 표가 그대로 나온다
```

⚠️ **데이터셋 검색은 키워드에 민감하다.** `마이홈포털`·`도로날씨`·`CCTV 도로날씨` 는 0건이었고
`임대주택단지`·`건축HUB` 는 잘 나왔다. 0건이 나오면 **API 가 없는 게 아니라 키워드가 안 맞는 것**일
수 있으니 단어를 바꿔 다시 검색할 것. 확실한 경로는 **마이페이지 활용신청 목록에서 항목을 클릭**해
상세로 들어가는 것이다.

### 7-1. 확인된 base URL (추측 금지 목록)

아래는 **실제로 확인한** 경로다. 다시 추측하지 말 것.

| 대상 | base URL | op | 상태 |
|---|---|---|---|
| 건축물대장 | `1613000/BldRgstHubService` | `getBr*` | ✅ 구현 |
| **건축인허가** | `1613000/ArchPmsHubService` | `getApBasisOulnInfo` `getApDongOulnInfo` `getApFlrOulnInfo` | ✅ 구현(2026-08-22) |
| 실거래 | `1613000/RTMSDataSvc{종류}` | `get{서비스명}` | ✅ 구현 |
| 소상공인 | `B553077/api/open/sdsc2` | `storeListInPnu` 외 11 | ✅ 구현 |
| 행안부 CCTV | `1741000/cctv_info/info` | — | ✅ 구현 |
| **LH 임대주택단지** | `B552555/lhLeaseInfo1/lhLeaseInfo1` | — | ⬜ 미구현. 필수 `ServiceKey`(**대문자 S**) `PG_SZ` `PAGE`, 옵션 `CNP_CD` `SPL_TP_CD`. **좌표 없음** → 지오코딩 필요 |
| **온비드 물건목록** | `B010003/OnbidRlstListSrvc2/getRlstCltrList2` | — | ⬜ 미구현(2026-08-22 **승인 확인**) |
| 온비드 물건상세 입찰정보 | `B010003/OnbidCltrBidDtlSrvc2/getCltrBidInf2` | — | ⬜ 필수 `cltrMngNo`+`pbctCdtnNo`. 개발계정 **일 1,000건** |
