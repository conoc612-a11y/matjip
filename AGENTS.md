...
> 6. 🔗 **[`API_SCHEMA.md`](API_SCHEMA.md) — API 를 붙이거나 값을 화면에 쓰기 전에 볼 것.**
>    각 API 의 **실측** 스키마, 서로를 잇는 조인 키(PNU·건물관리번호·지번), 그리고
>    **같은 값을 주는 다른 출처가 무엇인지**(교차검증 매트릭스)가 정리돼 있다.
>    항목마다 출처가 하나뿐이면 원본 오류가 그대로 화면에 나간다 — 실제로 터졌다
>    (실거래가 봉천동 862-1 을 `floor=80` 으로 주는데 실제는 지상 15층).
> 7. `git log -5`, `git status` 로 교차 확인
>
...
- **API 를 새로 붙이면 [`API_SCHEMA.md`](API_SCHEMA.md)에 실측 스키마를 적고 교차검증 매트릭스를 갱신**한다.
  추측으로 필드를 적지 말 것 — 실제 응답에서 옮겨 적고 채움률과 표본 크기를 밝힌다.
  기존 항목과 겹치는 값을 주는지 확인해, 겹치면 단일 출처를 교차검증으로 바꿀지 판단한다.
