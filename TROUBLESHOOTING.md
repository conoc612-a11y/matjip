...
- **국세청 사업자등록 진위확인**(구현됨)은 지금 **헤더의 독립 도구**다. 방금 붙인
  '이 건물 업소'와 **연결되지 않았다.**
  → 업소를 누르면 사업자 상태(계속/휴업/폐업)를 조회하는 흐름이 자연스럽다.
  ⚠️ **단, 국세청 API 는 사업자등록번호가 입력이다.** 소상공인 상가정보에는 사업자번호가 없다
  (§51-4 필드 39개에 없음). **상호→사업자번호 변환 경로가 공공데이터에 없다.**
  → 자동 연결은 **불가**. 사용자가 번호를 알 때 쓰는 도구로 남겨야 한다. 이 점을 오해하지 말 것.
