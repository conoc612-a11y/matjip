...
**읽는 순서**: `AGENTS.md`(구조) → 🔒 `LOCKED_POPUP_SPEC.md`(팝업 정본) → 이 문서(함정) → `HANDOFF.md`(직전 세션 상태) → `git log -5`

---

## ⚠️ 이 문서를 읽는 AI 에게 — 먼저 알아야 할 것

**이 문서에는 채택되지 않은 접근법도 섞여 있다.** 시간순 기록이라 "해결"이라고 적힌 뒤
나중에 되돌려진 항목이 있다. 그걸 모르고 베껴 쓰면 현재 코드가 깨지고, 같은 문제를
다시 "해결"하려 무한 반복하게 된다. 실제로 사용자가 이 문제를 지적했다(2026-08-20).

**규칙:**

1. 절 제목에 **⛔ 기각됨 / ⛔ 대체됨** 표시가 있으면 **그 해결책을 복원하지 마라.**
   해당 절들은 베껴 쓸 수 없게 **구현 코드를 의도적으로 삭제**했다. 원인 분석만 남겼다.
2. **팝업**(위치·크기·드래그·닫기/접기 버튼·스크롤바·그립)은 이 문서가 아니라
   🔒 **[`LOCKED_POPUP_SPEC.md`](LOCKED_POPUP_SPEC.md)** 가 정본이다. 어긋나면 LOCKED 가 맞다.
3. 문서의 값과 실제 코드가 다르면 **코드를 믿고, 문서를 고쳐라.** 문서에 맞춰 코드를
   되돌리지 마라. (실제 사례: §30-2 는 그립 `bottom` 을 음수로 적어놨지만 현재 코드는 `3px`
...
- **일시 차단 이력 (2026-08-18)**: 키 `143145cd464a4522b3a9347a9d768d4f`가ITS 사이트에서 401 반환 → its.go.kr 마이페이지에서 재등록 후 자동 해제(약 30분).
- **추가 서비스 등록 (2026-08-18)**: 돌발상황정보(`eventInfo`), 교통예측정보(`bypassFCastInfo`), 도로전광표지(`vmsInfo`), CCTV 화상자료(`cctvInfo` type=1). 현재 미사용 — 필요 시 land.html에 레이어 추가 가능.
...
> 🔒 **현재 확정 코드는 [`LOCKED_POPUP_SPEC.md`](LOCKED_POPUP_SPEC.md) §6 이다.** 아래는 원인 분석·이력이며,
> 값이 어긋나면 LOCKED 가 맞다. 코드를 되돌릴 때는 반드시 LOCKED 에서 가져올 것.
...
## 30. ⛔ 기각됨 — 팝업 리사이즈용 "스트립 핸들 + window click 차단" 접근법 (2026-08-16 시도, §30-1 에서 되돌림)

> **⛔ 이 절의 해결책은 채택되지 않았다. 구현을 복원하지 말 것.**
> 구현 세부(클래스명·플래그·리스너 배치)는 **의도적으로 삭제했다** — 베껴 쓰면 현재 코드가 깨진다.
> 현재 채택된 방식은 **§30-1**(`makeResizable`, pointer capture)이고, 관련 확정본은
> [`LOCKED_POPUP_SPEC.md`](LOCKED_POPUP_SPEC.md) 다.
>
> **현재 코드에 이 접근법의 흔적이 없음을 실측으로 확인**(2026-08-20): `lp-resize-handle` 0건,
> `rsDragging` 0건. 반면 `makeResizable` 12건. 즉 §30-1 이 실제 상태다.

남길 가치가 있는 것은 **원인 분석 하나뿐이다** (이건 지금도 유효하다):

- **원인(실측)**: 드래그 로직 자체는 정상 동작했다(높이 417→477 실변화). 문제는 mouseup 직후
  브라우저가 합성하는 `click` — mousedown 은 팝업, mouseup 은 지도 위라서 **공통 조상인 지도
  컨테이너에 click 이 발생** → `map.on('click')` 이 위치정보 팝업을 열고 → Leaflet 이 기존 팝업을
  닫으면서 새 팝업을 초기 크기로 되돌린다. 그래서 "끌어도 원래대로 돌아간다"처럼 보였다.
- **왜 지금은 이 문제가 없나**: `makeResizable` 이 pointerdown 에서 `setPointerCapture()` 를 걸어
  **드래그 후 합성 click 의 target 이 그립(=팝업 내부)에 머문다** → `disableClickPropagation` 이
  지도 click 전파를 막아 재오픈 경로가 **구조적으로 발생하지 않는다**. 즉 "window 캡처 click
  차단"은 pointer capture 가 없는 스트립 핸들 방식에만 필요했던 우회책이었다.
- **혼동 주의**: 이 절의 click 문제는 **팝업 리사이즈** 이야기다. 2026-08-20 에 고친
  **"지도를 드래그해 옮기면 팝업이 열린다"(§43)** 는 별개 문제이고 해결책도 다르다
  (지도 컨테이너 capture 단계 포인터 거리 측정). **두 개를 겹쳐 적용하지 말 것.**
...
## 30-2. ⛔ 대체됨 — 그립 위치를 아래로 매달아 겹침을 피하려던 시도 (2026-08-16, §37 로 대체)

> **⛔ 이 절의 해결값(그립을 팝업 밖으로 매다는 음수 bottom)은 현재 금지다.**
> 사용자 신고("팝업 창을 넘어가고 삐뚤어짐")로 되돌려졌고, **§37** 이 최종 해결이다:
> 그립은 우하단 코너 고정(`bottom:3px`), 겹침은 위치가 아니라 wrapper 의
> `padding-bottom:28px` 로 스크롤 영역을 그립 위에서 끝나게 해서 구조적으로 해결한다.
>
> **실측 확인(2026-08-20)**: 현재 CSS 는 `.leaflet-popup .ui-grip-corner { bottom: 3px; right: 3px; }`
> 다. 즉 이 절의 값은 코드에 없다. 아래 원문은 이력 보존용이며 **값을 복원하지 말 것.**
> (같은 날 `land.html` 안에도 금지 값을 현재 설정처럼 적어둔 낡은 주석이 발견돼 수정했다 —
> 문서와 코드 주석 양쪽에 같은 함정이 남아 있었다.)
...
- **당시 시도(⛔ 현재 금지 — CSS 선언은 베끼지 못하게 삭제함)**: popup 스코프에서 `.ui-grip-corner`
  의 `bottom` 을 **음수로** 줘 그립을 팝업 밖으로 매달았다. `css/buttons.css` 공용 값은 유지.
  → **§37 에서 폐기.** 현재 값은 `bottom: 3px`(코너 안쪽 고정)이며, 겹침은 wrapper 의
  `padding-bottom: 28px` 로 해결한다.
...
## 46. 법원 문서 버튼이 "이미 가진 데이터"를 두고 법원 사이트로 내보냈다 (2026-08-21)

- **증상(사용자)**: "바로가기 버튼 눌러서 확인하면 대법원 홈페이지 가서 사건번호를 수기로 입력해야 확인가능하게 나오거든" — 감정평가서·매각물건명세서를 보려면 매번 법원 사이트에서 사건번호를 손으로 입력해야 했다.
- **원인**: 데이터는 **이미 있었다.** `collect_auction_detail.js` 가 법원 응답
  `selectAuctnCsSrchRslt.on` 의 `dma_result` 에서 5개 하위 객체를 뽑아 `auction_detail.json` 에
  저장하고, `land.html` 상세 패널이 그걸 렌더까지 하고 있었다. 그런데
  ① 바로가기 라벨→섹션 매핑(`APD_LOCAL_GO`)에 **`사건내역`·`기일내역` 둘만** 등록돼 있었고
  ② 나머지 섹션은 `sec('', …)` 로 **id 가 없어** 스크롤 대상이 아예 없었다.
  그래서 데이터가 있어도 전부 법원 링크로 폴백됐다.
- **실측 보유 현황**(2026-08-21, 사건 2,601건 기준):

| 라벨 | 필드(법원 응답 키) | 내용 | 보유 |
|---|---|---|---|
| 사건내역 | `dspslGdsDxdyInfo` → `base`/`dspsl` | 사건번호·청구액·담당계 | 2,601 |
| 기일내역 | `gdsDspslDxdyLst` → `gihui` | 회차별 매각기일·최저가·결과 | 2,601 |
| **감정평가서** | `aeeWevlMnpntLst` → `evlt` | **감정평가 요항표**(구조·이용상태·설비·주의사항) | **2,590** |
| **매각물건명세서** | `gdsDspslObjctLst` → `objct` + `dspslGdsDxdyInfo.ndstrc` | 물건 목록·감정액 + 등기 인수사항 | 2,601 |
| (제시외물건) | `gdsNotSugtBldLsstAll` → `notsugt` | 창고·태양광 등 + 감정액 | 255 |
| 문건/송달 | — | **미수집** | 0 |
| 현황조사서 | — | **미수집** | 0 |

- **해결**: 섹션에 id 부여(`apd-sec-objct`/`apd-sec-notsugt`/`apd-sec-evlt`/`apd-sec-ndstrc`) +
  `APD_LOCAL_GO` 를 **라벨→섹션 후보 배열**로 바꿔 `감정평가서`·`매각물건명세서` 추가.
  법원 접속은 **한 번도 하지 않았다** — 순수 프론트 수정이다.
- **`detail` 존재만으로 판정하면 안 된다**: `evlt` 가 빈 사건이 **11건** 있다. 그때 앱에서
  열려는 시도를 하면 빈 화면이 된다. `aucLocalData(d, label)` 로 **필드별 실제 데이터 유무**를
  본다. 없으면 자동으로 법원 링크 버튼이 된다(실측 검증됨).
- **버튼 구분 표시**: 앱에서 열리는 버튼에 `.has-local`(테두리 강조 + `●`)을 붙였다. 어느 게
  법원으로 나가는지 미리 보여야 헛클릭이 줄어든다.

### 남은 일 — 문건/송달·현황조사서 (아직 조사 안 함)

`dma_result` 의 **전체 키를 덤프해 본 적이 없다.** 수집기는 5개만 뽑아 쓰고 나머지를 버린다.
거기에 문건/송달·현황조사가 함께 온다면 위와 똑같이 프론트 수정만으로 끝난다. 별도 탭
= 별도 API 라면 사건당 요청이 늘어 수집 시간이 배가 된다.

⚠️ **조사 시점 주의**: 사진/상세 수집기가 돌고 있는 동안에는 조사하지 말 것. 동일 IP 반복
요청으로 차단된 이력이 있다(§6-11, §6-13). 수집이 끝난 뒤 **사건 1건·요청 1회**로 확인하면 된다.

---

## 47. 감정평가서 PDF — **조사 완결. 우리 쪽에서 불가.** (2026-08-21, ⛔ 외부 차단)

> 사용자 요청: "바로가기 감정평가서 버튼 누르면 감정평가서 자료 바로 볼 수 있게 만들고 싶다.
> 다운로드 방식 / matjip 에서 링크 연결 방식 중 어느 쪽이 되는지 분석해달라."
>
> **결론: 둘 다 불가.** 원인은 우리 코드가 아니라 **한국감정평가사협회 뷰어가 IE 플러그인
> (ActiveX) 전용**이라 현대 브라우저에서 문서가 아예 렌더되지 않는 것이다.
> **이 절을 읽었으면 다시 조사하지 말 것.** 아래 "다시 하지 말 것" 목록에 시도·결과가 다 있다.
> 협회가 뷰어를 현대화하면 **"현대화되면 할 일"** 항목만 실행하면 끝난다(URL 조립법 확보됨).

### 47-1. 먼저 알아야 할 사실관계 (법원 FAQ 원문 근거)

법원경매정보 FAQ "감정평가서가 보이지 않습니다" 답변에 이렇게 적혀 있다(사용자 제공 캡처):

- 감정평가서를 열 때 **플러그인 프로그램을 설치**하게 되고, 설치하지 않으면 내용이 보이지 않을 수 있다.
- 안 보이면 **한국감정평가협회 담당자(02-3465-9870)** 에게 문의하라.
- **"이 홈페이지를 통하여 보실 수 있는 문서 중 감정평가서를 제외한 모든 문서는 법원에서
  제공하고, 감정평가서는 한국감정평가협회가 관리하는 서버의 정보를 협회 동의하에 링크하여
  보여드리는 것"**

→ 즉 **감정평가서만 외부(협회) 호스팅**이고, 나머지 문서는 법원 제공이다. 이 구분이 중요하다.

### 47-2. 확보한 것 — 문서 URL 조립법 (현대화되면 그대로 쓸 수 있다)

```
https://ca.kapanet.or.kr/view/{법원코드}/{사건번호}/{명령회차}/{평가서번호}/{작성일}
```

실측 검증 예 (서울중앙지방법원 2025타경103007):
```
https://ca.kapanet.or.kr/view/000210/20250130103007/1/2505006-13/20250609
```

**조각 ↔ API 필드 매핑 (1:1 확인됨)**

| URL 조각 | 값 | API 필드 |
|---|---|---|
| 법원코드 | `000210` | `cortOfcCd` = `"B000210"` → **맨 앞 `B` 제거** |
| 사건번호 | `20250130103007` | `csNo` (내부형식. 우리 DB 의 `base.saNo` 와 같은 형식) |
| 명령회차 | `1` | `ordTsCnt` |
| 평가서번호 | `2505006-13` | `aeeWevlNo` |
| 작성일 | `20250609` | `wrtYmd` |

**이 필드들의 출처**: `POST /pgj/pgj15B/selectAeeWevlInfo.on`
— **사건내역 화면에서 '감정평가서' 버튼을 누를 때** 발생한다(별도 화면 이동 없음).
응답 본문 예(579~700바이트):

```json
{"data":{"dlt_aeeWevlOrdTsLst":[{"cortOfcCd":"B000210","csNo":"20250130103007","ordTsCnt":1}],
 "dma_ordTsIndvdAeeWevlInf":{"cortOfcCd":"B000210","cortOfcNm":"서울중앙지방법원",
 "csNo":"20250130103007","ordTsCnt":1,"dspslPrcCrtrYmd":"20250529","exmnYmd":"20250528",
 "wrtYmd":"20250609","aeeEvlExamrNm":"이현진","aeeEvlJdgrNm":"김남빈",
 "aeeWevlNo":"2505006-13","fstmEvlDcsnOponCtt":"\"감정서 참조\"","userCsNo":"2025타경103007"}}}
```

⚠️ **옛 사건은 `aeeWevlNo` 형식이 다르다**: 2020타경111421 은 `서전210107C111421`(한글 포함).
URL 에 넣으려면 percent-encoding 이 필요하고, 그 사건에 협회 문서가 있는지도 미확인이다.

**부수 수확 — 지금 화면에 없는 유용한 값들**(PDF 와 무관하게 표시 가치 있음):
`dspslPrcCrtrYmd`(가격시점) · `exmnYmd`(조사일) · `wrtYmd`(작성일) ·
`aeeEvlExamrNm`(감정평가 담당자) · `aeeEvlJdgrNm`(심사자) · `aeeWevlNo`(평가서번호).

### 47-3. 버튼 위치 — 두 화면의 '감정평가서' 는 서로 다른 것이다 (내가 여기서 헛돌았다)

| 화면 | 버튼 id | 누르면 | 문서? |
|---|---|---|---|
| **사건내역**(PGJ159M00 검색결과) | `btn_aeeWevl` / `btn_aeeWevl2` | `selectAeeWevlInfo.on` + **협회 뷰어 모달** | ✅ 이쪽이 문서 경로 |
| 물건상세조회(PGJ15B) | `btn_aeeWevl1` / `btn_aeeWevl2` | `selectAeeWevlInfo.on` 만 | ❌ 메타데이터뿐 |

- 같은 `value="감정평가서"` 버튼이 **2개씩 있고 하나는 숨겨져 있다.** 숨은 걸 누르면 아무 일도
  안 일어난다 → **`isVisible()` 로 보이는 것만 클릭**해야 한다.
- 모달 id: `mf_wfm_mainFrame_aeeWevlPopUp`, 그 안 iframe id: `sbx_iframeTest` (src = 위 협회 URL).
- 현황조사서 버튼: `btn_curstExmndc` / `btn_curstExmndc2` — **조사 시 모달이 열리지 않았다(미확인)**.
  FAQ 상 법원 제공 문서이므로 협회와 다른 방식일 가능성이 크다. 별도 확인 필요.

### 47-4. ⛔ 다시 하지 말 것 — 시도와 결과 전부

| 시도 | 결과 |
|---|---|
| `el.click()` (JS 이벤트) 로 버튼 클릭 | **거부됨**(isTrusted=false). Playwright 실제 클릭 필요 (§6-11 과 같은 함정) |
| 응답 필터에 `viewer` 패턴 사용 | `/view/` 가 안 걸려 협회 요청을 **통째로 놓쳤다**. 필터 없이 전부 로깅할 것 |
| `/pdf/…`, `/file/…`, `/viewer/…` 경로 | **302 → `/error.htm`** (404 이미지) |
| URL 뒤에 `.pdf` 붙이기 | 같은 껍데기 HTML |
| `Referer: courtauction.go.kr` 추가 | 같은 껍데기 |
| IE UA (`MSIE 8.0`) 위장 | 같은 껍데기 |
| `Accept: application/pdf` | 같은 껍데기 |
| 응답 잘림 의심 → HTTP/1.0 · `--no-buffer` | **잘림 아님.** 항상 1,014~1,016B(±2 는 iframe name 자릿수) |
| 같은 세션으로 2·3차 재요청(iframe 재요청 가설) | 같은 껍데기. **쿠키가 아예 안 내려온다** |
| Playwright **headless / headful 둘 다** | 안쪽 iframe 끝까지 빈 상태. **PDF 요청 0건** |
| 협회 서버 루트 `/` 조회 | `alert("법원 경매 홈페이지를 통해서 이용해 주시기 바랍니다!")` + 법원으로 리다이렉트 |
| `robots.txt` | 302 → error |
| **사용자 실제 Chrome**(아크로뱃 설치까지 함) | **빈 화면** — 법원 페이지 모달에서도, URL 직접 열기에서도 |

🔴 **가장 중요한 함정 — HTTP 200 은 아무 의미가 없다.**
껍데기 페이지는 **파라미터를 검증하지 않는다.** 완전히 엉터리 값으로도 동일하게 200/1,016B 를 준다:
```
/view/000210/99999999999999/1/9999999-99/20990101  → 200  1016B
/view/000000/00000000000000/9/aaaa-bb/19000101     → 200  1016B
```
→ **URL 유효성을 상태코드로 판정하면 안 된다.** "200 이니 링크 붙이면 된다"고 결론 내렸다가
사용자 실제 확인에서 빈 화면으로 뒤집혔다. 검증은 **눈으로 문서가 보이는지**로만 해야 한다.

### 47-5. 왜 막혔나 (기술적 원인)

협회 뷰어 페이지는 **1,014~1,016바이트짜리 껍데기**다. 내용은 이게 전부다:
툴바(제목 + "아크로벳 다운받기" 배너) + `<iframe name="F{난수}" src="">`.
**`src` 가 비어 있고, 그걸 채우는 스크립트가 페이지에 없다.** jQuery 1.9 를 불러오지만
사용하는 코드가 없다. 서버는 IIS 7.5 / PHP 5.4 / ASP.NET 혼재.

즉 **iframe 을 채우던 주체가 브라우저 플러그인(ActiveX)** 이었고, 그 플러그인이 자체 경로로
문서를 가져갔다. 그래서 **HTML 어디에도 PDF 주소가 없다** → 우리가 받을 대상이 존재하지 않는다.
현대 브라우저는 그 플러그인을 지원하지 않으므로 **누가 열어도 빈 화면**이다.

### 47-6. ✅ 협회가 현대화되면 할 일 (구현 준비 완료 — 조사 불필요)

1. **수집기**(`tools/collect_auction_detail.js`)에 한 단계 추가:
   사건내역 화면에서 **보이는** `input[value="감정평가서"]` 클릭 → `selectAeeWevlInfo.on` 응답 캡처
   → `dma_ordTsIndvdAeeWevlInf` 에서 6개 필드 저장(`cortOfcCd`,`csNo`,`ordTsCnt`,`aeeWevlNo`,
   `wrtYmd` + 표시용 `dspslPrcCrtrYmd`,`exmnYmd`,`aeeEvlExamrNm`).
   **당사자내역(pgj15A) 캡처와 같은 페이지 방문에서 처리** → 추가 수집시간 사건당 2~3초뿐.
2. **URL 조립**은 47-2 표대로. 저장은 URL 문자열(약 80B) 또는 필드들. **추측 조립 금지** —
   `aeeWevlNo` 형식이 사건마다 다르다(한글 포함 사례 있음).
3. **프론트**: `aucLocalData(d,'감정평가서')` 판정에 이 URL 유무를 추가하고, 버튼이 새 창으로 URL 을 열게.
4. **다운로드까지 원하면**: 그때 개발자도구로 뷰어가 실제로 받는 PDF 요청을 다시 확인해야 한다
   (현대화되면 그 요청이 생길 것). 용량은 R2 무료 10GB 중 사진이 약 260MB 사용 중이라
   건당 1~5MB × 2,600건 = 2.6~13GB → **전량은 위험**, 진행 물건만 받는 쪽을 먼저 검토.
   ⚠️ 감정평가서 본문에는 **마스킹 없는 개인정보**가 있을 수 있다(당사자내역은 법원이 "김OO"로
   가려주지만 문서 본문은 다르다). 받아서 내용 확인 후 공개 범위를 판단할 것.

### 47-7. 확인할 곳 (다음 주 문의용)

- **한국감정평가사협회 02-3465-9870** (법원 FAQ 가 안내한 번호) —
  질문: ① 감정평가서 뷰어 플러그인이 최신 브라우저(Chrome/Edge)에서 동작하지 않는데
  **현대화(PDF 직접 서빙) 계획이 있는지** ② 있다면 시점 ③ `ca.kapanet.or.kr/view/...` URL 을
  **외부 서비스에서 링크로 참조해도 되는지**(FAQ 에 "협회 동의하에 링크"라고 적혀 있으므로 동의 범위 확인)
- **법원경매정보 고객센터** — 감정평가서 외 문서(현황조사서·매각물건명세서)는 법원 제공이라 했으니
  그쪽은 왜/어떻게 열리는지, 표준 URL 이 있는지.

### 47-8. 대안 (PDF 본문이 꼭 필요할 때)

- 유료 경매정보 사이트(지지옥션·탱크옥션 등)는 자체 확보한 PDF 를 제공한다.
- 우리가 이미 가진 것으로 상당 부분 대체된다: **감정평가 요항표(`evlt`)** = 구조·이용상태·
  설비·임대차 관계 요약(감정평가서 본문의 핵심 요약), **물건 사진**, **감정평가액·최저매각가격**.

---

## 48. 🔴 정정 — 법원 사이트는 해외 IP 차단이 **아니다** (2026-08-22 진단으로 확정)

**이 절은 §47 이전에 내가(Claude) 잘못 결론 낸 것을 바로잡는 기록이다.**
"courtauction.go.kr 은 GitHub Actions(해외 IP)에서 접속이 막힌다"고 커밋 메시지·TODO·
`update_auction.js` 헤더에 적었는데 **틀렸다.** 근거 없이 V-World 선례(§19-2)에 유추한 것이었다.

### 진단 결과 (Actions 러너에서 직접 측정, 대조군 포함)

러너: `20.106.191.87` (Virginia, US / AS8075 Microsoft)

| 사이트 | DNS | TCP 443 | TLS | HTTP GET |
|---|---|---|---|---|
| **www.courtauction.go.kr** | OK | **성공** | **OK (verify 0)** | **200 / 1.31s** |
| ↳ 수집기가 쓰는 SPA URL(`PGJ151F00.xml`) | — | — | — | **200 / 0.74s** |
| urban.seoul.go.kr (대조군A) | OK | 성공 | OK | 302 (정상) |
| api.vworld.kr (대조군B) | OK | 성공 | OK | **curl 52 Empty reply / http=000** |

→ **법원 사이트는 해외 IP 에서 정상 응답한다.** 네트워크·TLS·HTTP 어디서도 막히지 않는다.
→ 반면 **V-World 는 실제로 막힌다**(TCP·TLS 는 통과하고 HTTP 에서 응답 없이 끊음).
  §19-2 의 "ECONNRESET" 과 형태는 다르지만 같은 계열(애플리케이션/WAF 단 거부)이다.

### 그럼 워크플로는 왜 실패했나 — 실패 지점은 **SPA 초기화**다

실패 로그는 항상 `page.goto: Timeout 30000ms exceeded` ×3 → 이어서
`page.evaluate: Execution context was destroyed`. HTML 은 0.74초에 200 으로 오는데
**Playwright 의 page.goto 가 타임아웃**한다. 즉 HTML 이 아니라 **그 뒤 SPA 가 뜨는 과정**
(JS 번들·설정 XHR 로드 → 법원 셀렉트 렌더)에서 멈춘다. `openScreen()` 이 셀렉트를 15초
폴링하다 "화면 로드 실패 → 재시도"를 3회 내고 포기하는 것도 같은 지점이다.

### 같은 실패가 **로컬(한국 IP)에서도 재현됐다** — 결정적 근거

2026-08-22 15:28 로컬 실행: 서울중앙~7번째 법원까지 **2,148건 정상 수집** 후 갑자기
"화면 로드 실패 → 재시도" 반복 → 같은 지점에서 크래시(collect_auction.js:324).
직후 curl 로 같은 URL 을 치면 **200/0.2초로 멀쩡하다.**
→ IP·지역 문제가 아니라 **요청량에 따른 SPA 리소스 스로틀링**으로 보인다.
  이 사이트는 "폭주 시 차단"을 공지하고 있고(collect_auction.js 헤더), 그날 우리는
  수집기 2개 동시 실행 + 진단 여러 번으로 요청을 크게 늘렸다.

### 결론과 대응

- ❌ **"해외 IP 차단"이라고 쓰지 말 것.** 진단으로 반증됐다.
- ✅ 실패는 **SPA 초기화 단계**에서 나며, 요청량이 많을 때 잘 난다.
- ✅ **대응은 "쉬었다 다시"** — 실패하면 즉시 재시도하지 말고 시간을 두고 재실행한다.
  (즉시 재시도는 차단을 더 굳힌다. §6-11 과 같은 원칙)
- ⚠️ `collect-auction.yml` 은 삭제한 상태다. **되살릴 수 있는지는 미확정** —
  Actions 실패의 직접 원인(SPA 초기화 타임아웃)이 해외에서 구조적인지, 그냥 타임아웃을
  늘리면 되는지 확인되지 않았다. 되살리려면 러너에서 `--headful` 없이 page.goto 타임아웃을
  늘리고 셀렉트 폴링을 길게 잡아 한 번 시험해 볼 것.

### 내가 한 실수 (같은 함정을 밟지 말라고 남긴다)
1. **증상(닿지 않는다)에서 원인(IP 차단)으로 건너뛰었다.** V-World 선례가 있어 그럴듯했지만
   V-World 는 ECONNRESET 5/5 로 **직접 측정**해 확정한 것이고, 법원 건은 타임아웃뿐이었다.
...
## 50. 상호(세입자) 정보는 어느 DB에 있나 + 벡터지도 전환 검토 (2026-08-22 조사)

발단: 파리바게뜨가 있는 건물을 클릭했는데 팝업에 "단독주택"만 떴다. 그리고 "네이버는 상호에
마우스만 올려도 인식하는데 우리는 왜 안 되냐"는 질문. 두 질문의 답이 서로 얽혀 있어 같이 정리한다.

### 50-1. 왜 팝업에 상호가 없나 — 버그가 아니라 출처의 구조

팝업 이름·용도는 **역지오코딩 + 건축물대장(건축HUB)** 에서 온다. 이 장부에는
지번·주용도·연면적·구조·사용승인일은 있어도 **상호(임차 업소)가 아예 없다.**
관악로 232-1 의 법적 용도는 실제로 '단독주택'이 맞고, 1층 파리바게뜨는 다른 DB 에만 있다.

→ **대응 완료(2026-08-22, 커밋 `2a3ff73`)**: 건물명이 비어 있을 때 `findPois`(카카오 장소)를
자동 실행해 '이 부근 상호'를 팝업에 바로 띄운다. 배포본 실측 확인:
클릭 지점 용도 `단독주택` → `GS25 봉천래인점 편의점 · 6m` 자동 표시.

### 50-2. 상호·세입자 정보가 있는 DB (조사 결과)

| DB | 출처·근거 | 상호 | 층/호 | 갱신 | 한계 |
|---|---|---|---|---|---|
| **소상공인시장진흥공단 상가(상권)정보** | data.go.kr `15083033`(파일) / `15012005`(API). 소상공인시장진흥공단 | ✅ | 층정보 필드 **있다고 알려짐(미확인)** | **분기** | 폐업 반영이 최대 3개월 늦다. 사무실·병원 등 비상가 업종 누락 |
| 지방행정 인허가데이터개방(LOCALDATA) | 행정안전부. 인허가 원부 | ✅ | 소재지까지 | 일별 | **인허가 대상 업종만**(음식점·미용실 등). 사무실·일반 사무업 없음 |
| 카카오/네이버 장소 | 각 사 자체 구축 | ✅ | ❌ | 상시(가장 최신) | ToS상 저장·재배포 제약. 층·호 정보 없음 |
| 국세청 사업자등록(`NTS_API_KEY`) | 국세청 진위확인·상태조회 | ❌ | ❌ | 상시 | **주소로 목록 조회가 불가능하다.** 사업자번호를 주면 상태를 알려주는 API다(헤더 '사업자등록증조회'가 이것) |
| 건축물대장 전유부 | 건축HUB | ❌ | 층/호 있음 | — | **소유자·면적만. 상호 없음** |
| 등기부 임대차·확정일자 | 대법원 인터넷등기소 | 임차인명 | 있음 | — | **공개 API 없음.** 건별 유료 열람뿐 |
...
| 시도 | 결과 |
|---|---|
| 국세청 API 로 주소→사업자 목록 조회 | **불가.** 사업자번호 단건 조회 전용 |
...
일반지도는 V-World **WMTS 래스터 타일**(`.png`)이다. 라벨이 이미지 픽셀로 구워져 있어
DOM 요소도 좌표도 이름도 없다. **호버 인식은 미구현이 아니라 원리적으로 불가능하다.**

검토한 전환 경로와 실측 결과:

| 경로 | 결과 |
|---|---|
| V-World 벡터타일 `wmts/v1/{key}/{layer}/{z}/{y}/{x}.pbf` | **HTTP 500 / 본문 0바이트** (lt_l_sprd·lt_p_sprd·Base·POI 등 7개 시도 전부). 같은 키로 래스터 `Base.png` 는 200/10KB → 키 문제 아님 |
| V-World 공식 2D Data API 목록(158종) | **벡터타일 서비스가 목록에 없다.** XML/JSON 데이터 API 뿐 |
| 네이버/카카오 지도 SDK 로 교체 | **의미 없다.** 두 SDK 문서 모두 **개발자가 만든 마커·오버레이의 이벤트만** 다룬다. 기본지도에 그려진 POI 라벨을 객체로 노출하는 API·이벤트가 문서에 없다. 네이버 지도 웹사이트가 되는 것은 그들이 플랫폼 사업자라서이고, 공개 API 로 열어 준 기능이 아니다 |
| OSM 기반 벡터타일(MapTiler 등) | 호버는 되지만 **국내 상호 커버리지가 카카오/네이버보다 훨씬 나쁘다** — 지금보다 후퇴 |

**현실적인 대안(= 우리가 할 수 있는 최선)**: 상호를 **우리 마커로 직접 그린다.**
우리가 만든 마커니까 호버·클릭·툴팁이 당연히 된다. 단, 감수할 것들:
- V-World 타일에 구워진 라벨과 **위치가 어긋나 이중으로 보인다**
- 카카오는 **정해진 카테고리만** 조회된다(현재 `POI_CATS` 8종) → 타일 라벨 전부를 덮지 못한다
- 지도 이동마다 카테고리 수만큼 호출 → **디바운스·캐시·줌 하한(17 이상)** 이 필수
→ 그래서 "관심 카테고리 1~2개 토글"로 좁게 시작해 눈으로 확인한 뒤 넓히는 것을 권한다.
소상공인 API 를 붙이면 카테고리 제약 없이 **한 번 받아 두고** 그릴 수 있어 이 방식과 궁합이 더 좋다.

---

## 51. data.go.kr 승인 API 82건 전수 대조 — 무엇이 구현됐고 무엇이 남았나 (2026-08-22)

사용자 계정(conoc)의 **활용신청 현황 82건**(승인 79 · 신청중 3)을 실제 대시보드에서 확인하고
matjip 코드와 대조했다. 이전 메모의 "청약홈 3종 승인"은 **낡은 정보였다.**

전체 목록 확인법: 마이페이지 > 활용신청 현황(`/iim/api/selectAcountList.do`).
9페이지로 쪼개져 있으나 폼(`#searchVO`)에 `pageUnit=100` hidden input 을 넣고 submit 하면 한 번에 나온다.

### 51-1. ✅ 이미 구현된 data.go.kr API (13종)

| 승인 항목 | 구현 위치 |
|---|---|
| 건축HUB_건축물대장정보 (1613000 BldRgstHubService) | `molit-proxy` + land.html `ledgerOp` — op 11개 사용 |
| 아파트 매매 실거래가 **상세** (getRTMSDataSvcAptTradeDev) | `tools/collect_realprice.js` |
| 아파트 전월세 (AptRent) | 같음 |
| 연립다세대 매매·전월세 (RHTrade/RHRent) | 같음 |
| 단독/다가구 매매·전월세 (SHTrade/SHRent) | 같음 |
| 오피스텔 **전월세** (OffiRent) | 같음 |
| 청약홈 분양정보 (ApplyhomeInfoDetailSvc) | `chungak-proxy` |
| 국세청 사업자등록 진위확인·상태조회 (nts-businessman) | `bizno-proxy` |
| 기상청 단기예보 (1360000 VilageFcstInfoService_2.0) | `kma-weather-proxy` |
| 한국환경공단 전기차 충전소 (B552584 EvCharger) | `evcharger.json` |
| 행정안전부 CCTV정보 표준데이터 (1741000 cctv_info) | `tools/collect_cctv.js` |

⚠️ ITS CCTV(`openapi.its.go.kr:9443`)는 data.go.kr 이 아니라 국가교통정보센터다. 별개 키(`ITS_CCTV_KEY`).

### 51-2. ⭐ 승인됐는데 미구현 — matjip 에 값이 큰 것 (우선순위 순)

| 순위 | 승인 항목 | 왜 가치 있나 |
|---|---|---|
| 1 | **소상공인시장진흥공단_상가(상권)정보_API** | **세입자 리스트.** 실측 완료 — §51-4 |
| 2 | **한국자산관리공사_차세대 온비드 물건상세 입찰정보** | **공매.** 경매와 짝을 이루는 데이터인데 지금 전혀 없다 |
| 3 | **국토교통부_상업업무용 부동산 매매 실거래가** | 실거래가 지금 **주거용만** 있다. 상가·업무용이 빠져 상업 투자 판단이 불가 |
| 4 | **국토교통부_오피스텔 매매 실거래가** | 오피스텔은 **전월세만 받고 매매를 안 받는다**(비대칭 누락) |
| 5 | 국토교통부_공장 및 창고 등 매매 실거래가 | 위와 같은 공백 |
| 6 | 국토교통부_아파트 분양권전매 실거래가 | 분양권 시장이 빠져 있다 |
| 7 | 국토교통부_토지이용규제정보서비스 | 용도지역·행위제한. 지금은 V-World 로 일부만 커버 |
| 8 | 건축HUB 5종(폐쇄말소대장·건물에너지·주택인허가·건축인허가·건축물유지점검) | 신축 예정 파악 + 에너지등급 |
| 9 | 공동주택 관리비·장기수선충당금·하자담보·입찰공고 등 8종 | 아파트 실보유비용 — 투자 판단에 직결 |
| 10 | 한국부동산원 전국지가변동률 / 공동주택 단지 식별정보 / 녹색건축 | 지가 추세, 단지 코드 매칭 |
| 11 | LH·마이홈포털 6종(임대주택단지·분양임대공고·예비입주자 대기·공공임대 단지정보) | 공공주택 레이어 |
| 12 | 예금보험공사_파산금융회사 매물현황(부동산) | 특수 매물 |
| 13 | 건강보험심사평가원 병원정보 / 응급의료기관 / 착한가격업소 | 생활 인프라 레이어(맛집 탭과 연결) |

### 51-3. 🕒 아직 미구현 — 우선순위는 낮지만 **언젠가 구현할 것** (사용자 방침)

⚠️ **2026-08-22 정정**: 앞서 내가 이 목록을 "matjip 과 무관, 구현 안 해도 됨"으로 적었는데
**그건 내 판단이었고 사용자 방침과 다르다.** 사용자는 **언젠가 전부 구현할 계획**이며 활용안도 있다.
"무관"으로 취급해 논의에서 빼지 말 것.

사용자가 밝힌 활용안(그대로 옮긴다):
- **법무부 마을변호사** → 주소·변호사명으로 **지도에 아이콘화**하고 마을변호사 정보를 제공한다.
  (부동산 분쟁·계약 상담과 직결되므로 오히려 부동산 서비스와 잘 맞는다)
- **금융위원회 주식시세** → **헤더에 환율처럼** 띄운다. 부동산 투자자는 주식도 많이 하므로
  같은 자리에서 정보를 제공할 가치가 있다(환율·금리 위젯과 같은 성격).

나머지 미구현 목록(우선순위만 낮다):
경찰청 4종(습득물·분실물·핸드폰찾기·포털기관 습득물) · 해양수산부 2종(바다갈라짐·바다여행지수) ·
산림청 명산등산로 · 서울교통공사 최단경로 · 서울시설공단 방음벽 ·
공정거래위원회 가맹정보 3종 · 한국지역정보개발원 2종(부채규모·경영성과) · 해양수산부 포락지관리 ·
한국주택금융공사 전세자금보증상품 추천 · 행정안전부 공중화장실 · 국토교통부 정류장별 이용량 ·
행정안전부 공공데이터 제공표준 · 서울교육청 도서관 소장도서 · 조달청 나라장터 발주계획 ·
한국소비자원 생필품 가격 · 인천공항 출국장 혼잡도 · 국민연금 가입 사업장 · 금융위원회 기업재무 ·
보건복지부 2종(신청중) · 한국교통안전공단 주차정보(신청중)

💡 공정거래위원회 가맹정보 3종은 다시 보면 **상권 분석과 연결된다** — 방금 붙인 '이 건물 업소'의
브랜드가 가맹점인지, 그 본부의 재무가 어떤지까지 이어붙일 수 있다. 낮은 우선순위지만 무관하지 않다.

### 51-4. 소상공인 상가(상권)정보 — 실측 결과 (승인 확인 후, 2026-08-22)

**`storeListInPnu?key=<PNU>` 가 정답이다.** 파라미터명은 `key` 다(`pnu`·`lnoCd`·`cd` 는
`NO_MANDATORY_REQUEST_PARAMETERS_ERROR`).

파리바게뜨 건물(봉천동 34-4, PNU `1162010100100340004`) 실제 응답 — **3건, 그 건물만**:
...
팝업이 이미 갖고 있는 PNU 를 그대로 넘기면 된다(`pnuParts` 가 쓰는 그 값).
...
**채움률(반경 50m 36건 기준)** — 기대와 다른 부분이 있으니 UI 설계 전에 볼 것:

| 필드 | 채움 | 메모 |
|---|---|---|
| `lnoCd` (=**PNU 19자리**) | 36/36 | 우리 PNU 와 같은 체계 — 조인 키로 쓸 수 있다 |
| `bldMngNo` (건물관리번호 25자리) | 36/36 | `storeListInBuilding` 용. 우리는 필요 없다 |
| `flrNo` (층) | **28/36 (78%)** | **비는 경우가 있다.** '-층'으로 표시하거나 숨길 것 |
| `hoNo` (호) | **0/36** | 상가는 호를 안 적는다. **호 단위 표시는 포기할 것** |
| `dongNo` (동) | **0/36** | 같음 |
| `bldNm` (건물명) | 4/36 | 대부분 빈다 |

⚠️ **한계(사용자에게 표시할 때 감안)**: 갱신이 **분기**라 폐업이 최대 3개월 늦게 반영된다.
없어진 가게가 남아 있을 수 있으니 "○○년 ○분기 기준" 같은 표기가 필요하다.
또 상가업소 대상이라 **사무실·일반 사무업은 누락**된다.

⚠️ **`serviceKey` 를 프론트에 두지 말 것.** `molit-proxy` 와 같은 방식으로 Edge Function 프록시를
하나 더 만들어 중계한다(MOLIT_KEY 노출 사고와 같은 실수 반복 금지).

---

## 52. 승인 API 82건으로 matjip 을 어디까지 정확·풍부하게 만들 수 있나 — 분야별 분석 (2026-08-22)

§51 은 "무엇이 구현됐나"의 목록이다. 이 절은 **"그래서 정확도와 정보량을 어떻게 올리나"** 다.
사용자 요청으로 작성. 구현 순서를 정할 때 이 절을 근거로 삼을 것.

### 52-0. 지금 matjip 정보의 구조적 약점 3가지

분야별 이야기 전에, 82건을 훑어보고 나온 **관통하는 문제**부터 적는다. 개별 API 를 하나씩
붙이는 것보다 이 3가지를 고치는 게 체감이 크다.

| 약점 | 지금 상태 | 무엇이 문제인가 |
|---|---|---|
| **A. 실거래가 주거용만 있다** | 아파트·연립·단독·오피스텔(전월세) | **상업업무용·공장창고 매매가 없다.** 상가 건물을 클릭해도 "이 건물이 얼마에 거래됐나"를 못 본다. 경매 물건 중 상가·근린생활시설이 많은데 비교 기준이 없다 |
| **B. 건물의 '현재'를 모른다** | 건축물대장(준공 시점 사실) | 인허가·에너지·유지점검·폐쇄말소가 다 빠져 있다. **"이 옆에 뭐가 올라오는가"** 를 못 보여준다 — 투자 판단에서 가장 큰 변수인데 |
| **C. 아파트의 '보유 비용'을 모른다** | 시세·전세가율 | 관리비·장기수선충당금·하자·수의계약이 없다. **같은 가격이면 관리비 싼 단지가 낫다** 는 판단을 못 한다 |

### 52-1. 분야별 — 무엇을 붙이면 무엇이 좋아지나

#### ① 거래·시세 (정확도 ↑↑, 커버리지 ↑↑)

| API | 얻는 것 | 붙일 위치 |
|---|---|---|
| 상업업무용 부동산 매매 실거래가 | **상가·업무용 실거래.** 약점 A 의 핵심 | 팝업 '이 건물의 실거래' + 경매 물건 비교 |
| 오피스텔 매매 실거래가 | 전월세만 있는 비대칭 해소 | 같음 |
| 공장 및 창고 등 매매 실거래가 | 산업용 물건 | 같음 |
| 아파트 분양권전매 실거래가 | 분양권 시장 | 청약 탭과 연결 |
| 한국부동산원 전국지가변동률 | **추세.** 지금은 시점 값(공시지가)만 보여준다 | 팝업 공시지가 줄 옆에 "이 구 지가 +n%" |
| 한국부동산원 공동주택 단지 식별정보 | **단지 코드 표준.** 이름 매칭의 오매칭을 줄인다 | `rpGroupMap` 매칭 정확도 개선 |

⭐ **단지 식별정보는 "정확도" 관점에서 가장 값이 크다.** 지금 실거래↔건축물대장↔청약을
**단지명 문자열 포함 관계**로 붙이고 있다(land.html 주석에 '잠원동월드메르디앙' 사례가 있다).
표준 코드가 생기면 그 취약한 매칭을 대체할 수 있다.

#### ② 건물의 현재·미래 (약점 B)

| API | 얻는 것 |
|---|---|
| 건축HUB 건축인허가정보 | **착공 전 단계까지 보인다** — "옆 필지에 뭐가 허가났나" |
| 건축HUB 주택인허가정보 | 주택 공급 파이프라인 |
| 건축HUB 건물에너지정보 | 에너지 사용량 → 실제 관리비 추정, 노후도 판단 |
| 건축HUB 건축물유지점검정보 | 안전점검 이력 → 하자·리스크 |
| 건축HUB 폐쇄말소대장정보 | **철거된 건물** — 재개발 진행 확인 |
| 국토교통부 토지이용규제정보 | 행위제한 원문. 지금 V-World 용도지역만으론 부족 |

⭐ 이 6종은 **모두 같은 지번(PNU) 파라미터**를 쓴다. `ledgerOp` 가 이미 그 구조다
→ **op 이름만 늘리면 되고 새 인증·새 좌표 계산이 없다.** 투입 대비 산출이 가장 좋은 묶음.
⚠️ 단, 클릭당 호출이 늘어난다. 지금도 7회다. **'상세 보기' 클릭 때만** 부르는 현재 방식을 유지할 것.

#### ③ 아파트 보유 비용 (약점 C)

공동주택 관리비(개별사용료) · 관리비(장기수선충당금) · 하자담보책임기간 · 유지관리 이력 ·
입찰공고 · 입찰결과 · 수의계약 · 단지 목록 — **8종이 전부 승인돼 있다.**

얻는 것: "매매 14억, 전세가율 46%" 옆에 **"관리비 ㎡당 n원 · 장기수선충당금 잔액"**.
같은 가격대 단지를 고를 때 실제로 갈리는 지점이다.

#### ④ 공매 — 지금 완전한 공백

**한국자산관리공사 온비드 물건상세 입찰정보.** matjip 은 법원 경매를 3,442건 다루는데
**공매(온비드)는 0건**이다. 같은 사용자가 같은 목적으로 보는 데이터인데 절반이 없다.
게다가 온비드는 **공식 API 라 courtauction.go.kr 처럼 스크래핑할 필요가 없다** —
지금 경매 수집이 겪는 SPA 대기·차단 문제가 아예 없다.

⭐ **경매 파이프라인의 고생 대비, 공매는 훨씬 싸게 얻는다.** 우선순위를 높게 볼 이유.

#### ⑤ 공공주택·임대 (LH·마이홈 6종)

임대주택단지 · 분양임대공고 · 분양임대공고문 · 예비입주자 대기현황 · 공공주택 모집공고 ·
공공임대주택 단지정보. 청약 탭이 지금 **민간 분양(청약홈)만** 다룬다 → 공공임대·행복주택
수요자를 전혀 못 받는다.

#### ⑥ 사업자·상권 (방금 붙인 것의 확장)

- **국세청 사업자등록 진위확인**(구현됨)은 지금 **헤더의 독립 도구**다. 방금 붙인
  '이 건물 업소'와 **연결되지 않았다.**
  → 업소를 누르면 사업자 상태(계속/휴업/폐업)를 조회하는 흐름이 목표다.
  ⚠️ 국세청 API 는 사업자등록번호가 입력이고, 소상공인 상가정보 39필드에는 사업자번호가 없다.
  즉 **상호→사업자번호 연결 고리가 아직 없다.** 이것은 **미해결 과제이지 불가 판정이 아니다** —
  조사 경로와 진행 상황은 **§53** 에 따로 정리했다. 사용자 지시가 다시 오면 §53 부터 읽을 것.
- 공정거래위원회 가맹정보 3종 → 업소가 가맹점이면 본부·재무까지 이어붙일 수 있다.
- 행정안전부 착한가격업소 → 맛집 탭과 직결.

#### ⑦ 생활 인프라 (레이어 추가)

건강보험심사평가원 병원정보 · 국립중앙의료원 응급의료기관 · 행정안전부 공중화장실 ·
서울교육청 도서관 · 법무부 마을변호사(→ 지도 아이콘화, 사용자 방침 §51-3).
지금 CCTV·전기차충전소 레이어와 같은 방식으로 얹을 수 있다.

#### ⑧ 헤더 위젯

금융위원회 주식시세 → **환율처럼 헤더에** (사용자 방침 §51-3). 기존 `eximbank-proxy` 와 같은 패턴.

### 52-2. ⚪ CCTV 표준데이터 — **2026-08-23 폐기됨** (아래 회귀 기록은 이제 유효하지 않다)

> ⚠️ **이 절의 지침은 뒤집혔다.** 아래에 "표준데이터가 사라진 것은 회귀이니 되살려라"고 적혀
> 있지만, **2026-08-23 사용자 결정으로 표준데이터는 의도적으로 제외한다.**
> 이유: 그 데이터에는 **영상 스트림 URL 이 없다**(url 필드 0/39,484 실측). 위치·대수·보관일수만
> 있어서 카메라 아이콘을 눌렀는데 영상이 안 나오는 경험만 남았다. 사용자 표현: "영상 없는
> CCTV는 그냥 필요가 없다."
> → `cctv_static.json` 은 ITS 284건만(14.89MB → **74KB**). 수집기는 `WITH_STANDARD=1` 없이는
> 표준데이터를 받지 않는다.
> **아래 회귀 기록을 근거로 표준데이터를 되살리지 말 것.** 되살리려면 사용자에게 먼저 확인한다.
> 영상이 안 나오던 진짜 원인과 해결은 **§55** 를 볼 것.

#### (이하 2026-08-22 기록 — 역사로만 남긴다)

사용자 지적으로 발견. **가장 시급한 항목이다.** 새 API 를 붙이는 문제가 아니라 **이미 있던 것이 깨진 것.**

```
$ node -e "cctv_static.json 집계"
현재:      총 287건  {"its":287}          ← 표준데이터 0건
0f94bd5:   총 38,109건  {"standard":38109} ← ITS 0건
```

행안부 CCTV 표준데이터 API 는 지금도 정상이다(실측 HTTP 200, **전국 totalCount 377,278**).

**원인 — 과거 '수정'이 데이터를 지웠다.** 커밋 이력이 그대로 말해 준다:

| 커밋 | 내용 | 결과 |
|---|---|---|
| `d74d8f3` | 표준데이터 54,238건 수집 | standard 54,238 |
| `0f94bd5` | 서울 외 주소 필터 | standard 38,109 |
| `b641bc1` | "ITS 영상 데이터 복구 — 재수집 (287건)" | **its 287, standard 0** ← 38,109건 소멸 |

`tools/collect_cctv.js` 는 ITS + 표준을 **병합하도록 설계돼 있다**(`main()` 이 둘 다 push).
그런데 실제 실행은 매번 `--its-only` / `--standard-only` 플래그로 돌아서 **서로를 덮어썼다.**
두 소스가 한 파일에 함께 있었던 적이 **한 번도 없다.**

**게다고 CCTV 는 워크플로가 없다**(`.github/workflows` 에 collect-cctv.yml 이 없다) — 수동 실행뿐이라
아무도 되돌리지 않았다.

**조치**: 플래그 없이 `node tools/collect_cctv.js` 를 돌리면 둘 다 채워진다.
⚠️ **다시 하지 말 것**: `--its-only` / `--standard-only` 로 돌린 결과를 **그대로 커밋하지 말 것.**
그 플래그는 디버깅용이다. 커밋 전에 `cctv_static.json` 의 source 별 건수를 확인할 것.

**추가로 붙일 수 있는 CCTV**: 기상청_CCTV 기반 도로날씨정보(승인됨, 미구현) — 도로 상태 영상.
ITS(`openapi.its.go.kr`)는 data.go.kr 이 아닌 별개 키이며 이미 구현돼 있다.

### 52-3. 우선순위 (투입 대비 산출)

| 순위 | 항목 | 근거 |
|---|---|---|
| 1 | **CCTV 회귀 복구** | 새 개발 0. 명령 하나로 287 → 38,000+. 이미 있던 기능 |
| 2 | **온비드 공매** | 경매의 빈 절반. 공식 API 라 스크래핑 고생이 없다 |
| 3 | **상업업무용·오피스텔·공장창고 매매 실거래** | 기존 `collect_realprice.js` 에 op 추가. 상가 판단의 전제 |
| 4 | **건축HUB 6종** | `ledgerOp` 에 op 이름만 추가. 새 인증·좌표계산 없음 |
| 5 | **공동주택 단지 식별정보** | 취약한 문자열 매칭을 표준 코드로 대체 = 정확도 |
| 6 | 공동주택 관리비 8종 | 보유비용 정보. 붙일 자리가 이미 있다(아파트 팝업) |
| 7 | LH·마이홈 6종 | 청약 탭 확장 |
| 8 | 생활 인프라 레이어 + 헤더 주식시세 | 기존 레이어·위젯 패턴 재사용 |

⚠️ **공통 제약**: data.go.kr 은 **계정당 인증키가 공용**이고 개발계정은 일 10,000건이다.
`molit-proxy`·`kma-weather-proxy`·`sbiz-proxy` 가 이미 `datagokr:` 레이트리밋 버킷을 공유한다.
새 프록시를 만들 때 **반드시 같은 버킷을 쓸 것.** 함수별로 따로 세면 합계가 한도를 넘긴다.
클릭당 호출 수가 늘어나는 항목은 **'상세 보기' 클릭 때만** 부르는 현재 방식을 유지할 것.


---

## 53. 🟠 미해결 — '이 건물 업소' → 사업자등록 정보 연결 (상호→사업자번호 고리 찾기)

**이건 "불가"가 아니라 "아직 못 찾았다"다.** 사용자 방침: **방법을 찾아야 한다.**
앞서 내가 "자동 연결 불가, 번호를 알 때 쓰는 도구로 남겨야 한다"고 못박았는데 **그렇게 닫지 말 것.**
사업자등록 관련 지시가 다시 오면 이 절부터 읽고, 아래 시도 기록에 이어서 진행할 것.

### 53-1. 목표

팝업 '이 건물 업소'에서 업소를 누르면 → **사업자 상태(계속/휴업/폐업) · 개업일 · 대표자**까지 보여준다.
가치: 소상공인 상가정보는 **분기 갱신이라 폐업이 최대 3개월 늦다**(§51-4). 국세청 상태조회는 실시간이라
**"이 가게 아직 영업하나"를 그 자리에서 확인**할 수 있다. 상가 투자·경매 물건 판단에 직접 쓰인다.

### 53-2. 왜 막혀 있나 (구조)

| 갖고 있는 것 | 필요한 것 |
|---|---|
| 상호명 · 지번 · 층 · 업종 (소상공인, §51-4) | **사업자등록번호 10자리** |
| 국세청 상태조회 API (구현됨, `bizno-proxy`) | 입력이 **사업자등록번호**뿐 |

→ **상호+주소 → 사업자등록번호** 변환이 빠진 고리다.

### 53-3. 시도 기록 (2026-08-22) — 이어서 할 것

#### ⏳ 국민연금공단_국민연금 가입 사업장 내역 (승인됨) — 가장 유력, 미완

사업장 데이터에 **사업자등록번호 필드(`bzowr_rgst_no`)** 가 있다고 알려져 있다. 있으면 고리가 이어진다.

동작하는 엔드포인트를 찾았다:
```
✅ https://apis.data.go.kr/B552015/NpsBplcInfoInqireServiceV2/getBassInfoSearchV2
   → <resultCode>00</resultCode> NORMAL_CODE
❌ B552015/NpsBplcInfoInqireService/getBassInfoSearch      → NO_OPENAPI_SERVICE_ERROR
❌ B552015/NpsBplcInfoInqireService/getBassInfoSearchV2    → NO_OPENAPI_SERVICE_ERROR
```

**그런데 어떤 파라미터로도 totalCount 0 이다.** 시도한 것 전부 0:
`wkpl_nm=파리바게뜨` · `ldong_addr_mgpl_dg_cd=11&ldong_addr_mgpl_sggu_cd=11620` ·
`...sggu_cd=620` · `data_crt_ym=202606` · `data_crt_ym=202607` · `seq=1` · 파라미터 없음

**다음에 할 일**: data.go.kr `3046071`(오픈API) 상세 페이지의 **'상세기능 → 요청변수'** 표를
로그인 상태에서 직접 읽어 정확한 파라미터명·필수값을 확인한다. 검색 결과 요약으로는 확정이 안 됐다.
(사용자 크롬으로 접속하면 볼 수 있다 — §51 목록을 그렇게 확인했다.)

⚠️ **미리 알아 둘 한계**: 이 데이터는 **가입자 3인 이상 법인 / 10인 이상 개인사업자**만 수록한다.
소규모 상가(파리바게뜨 가맹점 등)는 **대부분 빠진다.** 즉 이 경로가 열려도 **커버리지는 부분적**이다.
그래도 규모 있는 업소·법인은 잡히므로 가치가 없지는 않다.

#### 아직 안 본 경로 (후보)

| 경로 | 왜 볼 만한가 | 확인할 것 |
|---|---|---|
| 지방행정 인허가데이터(LOCALDATA) | 음식점·미용실 등 **인허가 업소**에 사업자정보가 붙는 경우가 있다 | 응답에 사업자등록번호가 실제로 오는지. **미신청** 상태라 신청 먼저 |
| 공정거래위원회_가맹정보 3종 (승인됨) | 가맹**본부**는 법인이라 사업자번호가 있을 가능성 | 가맹점 개별까지 오는지(아마 본부만) |
| 금융위원회_기업 재무정보 (승인됨) | 법인 식별정보 | 상장·외감 법인 한정일 것 |
| 국세청 휴·폐업자 명단 공개자료 | 국세청이 파일로 공개한다고 알려져 있다 | data.go.kr 에 있는지, 상호·주소가 함께 오는지 |

#### ⛔ 확인된 막다른 길 (다시 하지 말 것)

| 시도 | 결과 |
|---|---|
| 국세청 상태조회 API 에 상호·주소를 넣어 목록 조회 | **불가.** 입력이 사업자등록번호 단건뿐(`nts-businessman/v1/status`) |
| 소상공인 상가정보에서 사업자번호 찾기 | **없다.** 39필드 전수 확인(§51-4) |

### 53-4. 고리를 못 찾더라도 할 수 있는 것 (차선)

연결이 안 되어도 **팝업에서 국세청 조회로 넘기는 흐름**은 만들 수 있다:
업소 옆에 조회 버튼 → 헤더의 사업자등록증 조회 모달을 **상호를 채운 상태로** 열어 준다.
번호는 사용자가 넣지만, "이 가게 확인해보자"의 동선이 끊기지 않는다.
⚠️ 이건 차선이고 목표가 아니다. **§53-3 을 먼저 끝까지 볼 것.**

---

## 54. 🔴 실거래 수집기가 API 오류를 '거래 0건'으로 삼켜 서울 12개 구를 잃었다 (2026-08-22)

**증상**: 비주거 실거래 첫 수집이 `exit 0` 으로 정상 종료하고 2,261곳/12,467건을 저장했는데,
**서울 12개 구(은평·서대문·마포·양천·강서·구로·금천·영등포·동작·관악·서초·강남)와 경기 34개
지역이 통째로 없었다.** 팝업에서 관악구 건물을 클릭해도 거래가 안 나와서 발견했다.

```
$ node -e "키 앞자리 집계"
서울 확보: 종로 중 용산 성동 광진 동대문 중랑 성북 강북 도봉 노원 송파 강동  (13개)
서울 누락: 은평 서대문 마포 양천 강서 구로 금천 영등포 동작 관악 서초 강남  (12개)
```

직후 같은 요청을 단건으로 치면 **정상 200 + 데이터가 나온다** → 데이터가 없는 게 아니었다.

### 54-1. 원인 ① 수집기가 오류를 0건으로 해석했다 (진짜 원인)

`parseItems()` 는 `<item>` 이 없으면 `{items:[], total:0}` 을 돌려줬다. data.go.kr 은
**쿼터 초과·스로틀링에도 HTTP 200 에 오류 XML** 을 실어 보내므로, `fetchAll` 은 그걸
"그 구·그 달은 거래가 없다"로 읽고 `break` 했다. 그래서 실패가 **로그에도 남지 않았다.**

유일한 흔적은 진행 로그의 누적 건수였다:
```
[상업업무용] 250/864 (누적 5,412건)
[상업업무용] 300/864 (누적 5,412건)   ← 100개 작업 동안 한 건도 안 늘었다
[상업업무용] 350/864 (누적 5,412건)
```

⚠️ `writeSafe` 가드도 못 막았다 — **첫 수집이라 비교할 기존 파일이 없었다.**
가드는 "기존 대비 절반 미만"만 본다. 첫 파일에는 무력하다.

### 54-2. 원인 ② 같은 인증키로 두 수집기를 동시에 돌렸다 (내 실수)

CCTV 수집기(전국 377,278건 → **3,773 요청**)와 비주거 실거래 수집기(**2,500+ 요청**)를
**동시에** 돌렸다. 둘 다 `DGK` 를 쓴다. **data.go.kr 은 계정당 인증키가 공용**이고 개발계정은
일 10,000건이라, 합쳐서 한도를 밀어붙여 중간 구간이 스로틀링을 맞았다.

이미 같은 교훈이 있었다 — 경매 스크레이퍼 2개 동시 실행이 서로의 데이터를 지운 사고(§48 주변).
**이번은 그 API 쿼터 버전이다.**

### 54-3. 조치 (커밋 참조)

**`parseItems` 가 오류를 구분해 올려보낸다:**
- `<errMsg>` / `<returnAuthMsg>` 있으면 → `err`
- `<resultCode>` 가 `0`/`00`/`000` 이 아니면 → `err`
- 응답 껍데기(`<response|<items|<totalCount`)조차 없으면 → `err` (빈 본문·HTML 오류페이지)

**`fetchAll` 이 재시도하고, 끝까지 실패하면 크게 실패한다:**
- 간격 `0 → 3s → 9s → 20s`. 넉넉히 두는 이유: 스로틀링은 잠깐 멈춰야 풀린다.
  바로 재시도하면 구멍을 더 깊게 판다(§0-A 경매 사이트에서 겪은 것과 같은 성질).
- 그래도 실패하면 실패 목록을 찍고 **throw** → 부분 결과를 저장하지 않는다. 기존 파일은 보존된다.

### 54-4. ⛔ 다시 하지 말 것

| 하지 말 것 | 이유 |
|---|---|
| **DGK 를 쓰는 수집기를 두 개 이상 동시에 실행** | 계정당 키 공용. 합계가 한도를 넘겨 중간 구간이 조용히 비워진다 |
| 응답에 `<item>` 이 없는 것을 '0건'으로 처리 | 오류와 구분되지 않는다. 이 사고의 직접 원인 |
| 첫 수집 결과를 `writeSafe` 통과만 보고 신뢰 | 비교할 기존 파일이 없어 가드가 무력하다. **지역 커버리지를 직접 세어 볼 것** |
| 실패 후 즉시 재시도 | 스로틀링을 악화시킨다 |

### 54-5. 수집 후 반드시 확인할 것 (첫 수집·전량 재수집 시)

```bash
node -e "
const j=require('./realprice_nonres.json');
const have=new Set(Object.keys(j).map(k=>k.split(' ')[0]));
const seoul=['종로구','중구','용산구','성동구','광진구','동대문구','중랑구','성북구','강북구','도봉구','노원구','은평구','서대문구','마포구','양천구','강서구','구로구','금천구','영등포구','동작구','관악구','서초구','강남구','송파구','강동구'];
console.log('누락:', seoul.filter(g=>!have.has(g)).join(', ') || '없음');
"
```
서울 25개 구가 전부 나와야 정상이다. 다른 realprice_*.json 도 같은 방식으로 셀 수 있다.

---

## 55. 🟢 CCTV 영상이 안 나온 진짜 원인 — mixed content (2026-08-23 해결)

**증상**: CCTV 마커를 눌러도 영상이 안 나온다. 검은 화면만 남는다.

**처음에 잘못 짚은 것**: 나는 "영상 없는 데이터라서"로 설명하고 **레이어 제거를 제안했다.**
사용자 지적으로 다시 봤다 — 영상은 정상 제공되고 있었다. 문제는 우리 코드였다.
⛔ **문제를 만나면 삭제·회피가 아니라 해결책을 찾을 것.**

### 55-1. 원인 (실측)

ITS 가 주는 `cctvurl` 은 **전부 http** 다(284/284). matjip 은 https 로 배포되므로
`<video src="http://…">` 는 브라우저가 **mixed content 로 차단**한다. 영상 자체는 멀쩡했다:

```
$ curl -sIL "http://cctvsec.ktict.co.kr/99/…"
→ 302 → http://cctvsec.ktict.co.kr:8081/….mp4   → 200 · video/mp4 · 3,657,595 bytes
```

### 55-2. 해결 — 스킴만 바꾸면 된다 (프록시 불필요)

같은 URL 을 **https 로 요청하면 전 구간이 https 로 유지된다**(실측):

```
$ curl -sI "https://cctvsec.ktict.co.kr/99/…"
→ 302  Location: https://cctvsec.ktict.co.kr:8082/….mp4     ← https 전용 포트 8082
   최종 200 · video/mp4 · 3,716,578 bytes (리다이렉트 2회, 전부 https)
```

`http://` 로 요청하면 **8081**, `https://` 로 요청하면 **8082** 로 간다. 별도 https 포트가 있다.
→ `httpsStream(url)` 로 스킴만 치환한다(land.html). **Edge Function 프록시가 필요 없다.**

⚠️ **되돌리지 말 것**: http 로 두면 배포본에서 조용히 차단된다. 로컬 http 개발에선 재생되므로
"잘 되는데?" 로 오판하기 쉽다.

⚠️ 코드 주석의 "ITS 가 Edge Function IP 를 막는다"는 **목록 API**(`openapi.its.go.kr:9443`)
이야기다. **스트림 호스트(`cctvsec.ktict.co.kr`)는 다른 서버**이고 막지 않는다 — 이 둘을 섞어서
"프록시가 불가능하다"고 판단하지 말 것(내가 그렇게 오판했다).

### 55-3. 두 번째 문제 — 저장된 URL 은 토큰이 만료돼 있다

스트림 URL 에 `wmsAuthSign` 서명이 붙고, 디코드하면 `validminutes=120` 이다.
즉 `cctv_static.json` 에 저장해 둔 URL 은 **배포 직후를 빼면 항상 죽어 있다.**

해결: 마커는 정적 데이터로 그리되 **클릭 시점에 ITS 를 다시 물어 fresh URL 로 재생**한다
(`freshCctvUrl()`). live 로 받은 항목은 `_live` 로 표시해 재조회를 건너뛴다.
bbox 는 ±0.004°(약 400m) — 더 좁히면 ITS 가 0건을 준다(실측).

### 55-4. 실패를 조용히 넘기지 않는다

예전엔 `video.src = url` 로 끝이라 차단·만료·장애가 **모두 검은 화면**으로만 보였다.
지금은 `error` 이벤트와 12초 타임아웃을 받아 원인 문구 + '다시 시도' 버튼을 띄운다.
`MEDIA_ERR_SRC_NOT_SUPPORTED`(code 4) 는 토큰 만료 안내로 연결한다.

### 55-5. ⛔ 다시 하지 말 것

| 시도 | 결과 |
|---|---|
| CCTV 레이어 제거로 회피 | **하지 말 것.** 해결 가능한 문제였다 |
| 스트림용 Edge Function 프록시 | **불필요.** https 스킴 치환으로 끝난다 |
| `cctv_static.json` 의 url 로 바로 재생 | 토큰 만료. 클릭 시 fresh 조회가 필요하다 |
| 행안부 표준데이터 되살리기 | 영상이 없다(§52-2 는 폐기된 지침) |
