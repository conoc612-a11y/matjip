 ...
 * ⚠️ 2026-08-22 정정: 처음엔 "해외 IP 차단"이라고 적었는데 **틀렸다**(TROUBLESHOOTING §48).
 * 진단 결과 Actions 러너에서도 법원 사이트는 HTTP 200 으로 정상 응답한다. 실패 지점은
 * 네트워크가 아니라 **SPA 초기화**(page.goto 타임아웃)이고, 요청량이 많을 때 잘 난다.
 * 로컬에서도 같은 실패가 재현됐다. 그래도 로컬 실행을 기본으로 두는 이유는 아래와 같다.
 * Actions 실패 로그: `page.goto: Timeout 30000ms exceeded` ×3 → 수집 실패.
 * 2026-08-15~21 매일 실패/취소, **성공 이력 0회**. 원인이 확정되지 않아(§48) 자동 실행을
 * 포기하고 **사람이 확인하며 돌리는 로컬 실행**으로 옮겼다. collect-auction.yml 은 그래서 삭제했다.
 * 되살릴 수 있는지는 §48 의 "되살릴 수 있는지" 항목 참고.
 *
 * ⚠️ 로컬에서도 요청량이 많으면 같은 실패가 난다(2026-08-22: 2,148건 수집 후 발생).
 * 실패하면 **즉시 재시도하지 말고 시간을 두고** 다시 돌릴 것 — 즉시 재시도는 상황을 악화시킨다.
 *
 * ── 왜 하나로 묶었나 ────────────────────────────────────────────────────
